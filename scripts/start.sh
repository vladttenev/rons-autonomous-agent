...
Rons is an open-source AI agent.
            ...
            "Use Existing")
                ...
                case "$action" in
                    ...
                    "Run")
                        ...
                        echo "$selected_names" > /tmp/rons_selected_names
                        ...
                        done < /tmp/rons_selected_names
                        rm -f /tmp/rons_selected_names
...
start_rons() {
    ...
    if ! kill -0 $SERVER_PID 2>/dev/null; then
        log_error "Failed to start Rons server"
        return 1
    fi
    ...
    if ! kill -0 $CLIENT_PID 2>/dev/null; then
        log_error "Failed to start Rons client"
        kill $SERVER_PID 2>/dev/null
        return 1
    fi

    # Open in browser
    log_info "Opening Rons in your browser..."
    ...
    log_success "Rons is now running"
    log_info "Press Ctrl+C to stop Rons"
                ...
                case "$action" in
                    ...
                    "Update")
                        ...
                        if [ "$LOCAL" = "$REMOTE" ]; then
                            ...
                            # Ask to start
                            if gum confirm "Would you like to start Rons now?"; then
                                start_rons
                                exit 0
                            fi
                        ...
                        log_info "Updates available. Updating Rons..."
...
main() {
    ...
    if ! gum confirm "Ready to install Rons?"; then
    ...
    log_info "Rons has been stopped"
}
