...
case "$1" in
    build)
        docker build --platform linux/amd64 -t rons .
        ;;
    run)
        # Ensure the container is not already running
        if [ "$(docker ps -q -f name=rons)" ]; then
            echo "Container 'rons' is already running. Stopping it first."
            docker stop rons
            docker rm rons
        fi
    ...
    bash)
        ...
        if [ "$(docker ps -q -f name=rons)" ]; then
            docker exec -it rons bash
        else
            echo "Container 'rons' is not running. Please start it first."
            exit 1
        fi
        ;;
    *)
