...
class DeriveKeyProvider {
    ...
    private async generateDeriveKeyAttestation(
        ...
        ronsLogger.log("Remote Attestation Quote generated successfully!");
        return quote;
    }
    ...
    async rawDeriveKey(
        ...
        try {
            ...
            ronsLogger.log("Deriving Raw Key in TEE...");
            const derivedKey = await this.client.deriveKey(path, subject);

            ronsLogger.log("Raw Key Derived Successfully!");
            return derivedKey;
        } catch (error) {
            ronsLogger.error("Error deriving raw key:", error);
            throw error;
        }
    }
    ...
    async deriveEd25519Keypair(
        ...
        try {
            ...
            ronsLogger.log("Deriving Key in TEE...");
            const derivedKey = await this.client.deriveKey(path, subject);
            ...
            ronsLogger.log("Key Derived Successfully!");

            return { keypair, attestation };
        } catch (error) {
            ronsLogger.error("Error deriving key:", error);
            throw error;
        }
    }
    ...
    async deriveEcdsaKeypair(
        ...
        try {
            ...
            ronsLogger.log("Deriving ECDSA Key in TEE...");
            ...
            ronsLogger.log("ECDSA Key Derived Successfully!");

            return { keypair, attestation };
        } catch (error) {
            ronsLogger.error("Error deriving ecdsa key:", error);
            throw error;
        }
    }
}
...
const deriveKeyProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message?: Memory, _state?: State) => {
        ...
        try {
            ...
            ronsLogger.error("Error in derive key provider:", error.message);
