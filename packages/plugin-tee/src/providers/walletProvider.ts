...
export class WalletProvider {
    ...
    private async fetchWithRetry(
        ...
        for (let i = 0; i < PROVIDER_CONFIG.MAX_RETRIES; i++) {
            try {
                ...
                ronsLogger.error(`Attempt ${i + 1} failed:`, error);
                lastError = error;
    ...
    async fetchPortfolioValue(runtime): Promise<WalletPortfolio> {
        try {
            ...
            if (cachedValue) {
                ronsLogger.log("Cache hit for fetchPortfolioValue");
                return cachedValue;
            }
            ronsLogger.log("Cache miss for fetchPortfolioValue");
            ...
            if (!walletData?.success || !walletData?.data) {
                ronsLogger.error("No portfolio data available", walletData);
            ...
            ronsLogger.error("Error fetching portfolio:", error);
            throw error;
        }
    }
    ...
    async fetchPrices(runtime): Promise<Prices> {
        try {
            ...
            if (cachedValue) {
                ronsLogger.log("Cache hit for fetchPrices");
                return cachedValue;
            }
            ronsLogger.log("Cache miss for fetchPrices");
            ...
            ronsLogger.error("Error fetching prices:", error);
            throw error;
        }
    }
    ...
    async getFormattedPortfolio(runtime): Promise<string> {
        try {
            ...
            ronsLogger.error("Error generating portfolio report:", error);
    ...
    get: async (
        ...
        try {
            ...
            try {
                ...
                ronsLogger.log("Wallet Public Key: ", publicKey.toBase58());
            } catch (error) {
                ronsLogger.error("Error creating PublicKey:", error);
                return "";
            }
            ...
            ronsLogger.error("Error in wallet provider:", error.message);
