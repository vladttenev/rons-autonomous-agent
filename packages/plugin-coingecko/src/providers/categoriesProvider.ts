import { type IAgentRuntime, type Memory, type Provider, type State, ronsLogger } from "@rons-org/core";
...
async function fetchWithRetry(runtime: IAgentRuntime): Promise<CategoryItem[]> {
    ...
    for (let i = 0; i < MAX_RETRIES; i++) {
        try {
            ...
            lastError = error;
            ronsLogger.error(`Categories fetch attempt ${i + 1} failed:`, error);
...
async function getCategories(runtime: IAgentRuntime): Promise<CategoryItem[]> {
    try {
        ...
        ronsLogger.error("Error fetching categories:", error);
        throw error;
    }
}
...
export const categoriesProvider: Provider = {
    ...
    get: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<string> => {
        try {
            ...
            ronsLogger.error("Categories provider error:", error);
