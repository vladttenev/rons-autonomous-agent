...
export class Arbitrage {
    ...
    async evaluateMarkets(marketsByToken: MarketsByToken): Promise<CrossedMarketDetails[]> {
        ronsLogger.log("Starting market evaluation...");
        ...
        for (const market of markets) {
            for (let attempt = 1; attempt <= maxAttempts; attempt++) {
                try {
                    ...
                    if (transaction) {
                        ronsLogger.log(`Successful arbitrage execution: ${transaction.hash}`);
    ...
    async submitBundleWithAdjustedGasPrice(bundle: BundleEntry[], blockNumber: number, blocksApi: any): Promise<void> {
        ronsLogger.log(`Submitting bundle with adjusted gas price for block ${blockNumber}`);
        ...
        try {
            ...
            ronsLogger.log("Bundle submitted successfully:", {
    ...
    private async adjustGasPriceForTransaction(
        ...
        ronsLogger.log("Calculating adjusted gas price", {
        ...
        ronsLogger.log("Adjusted gas price:", adjustedGasPrice.toString());
    ...
    private async calculateOptimalVolume(
        ...
        ronsLogger.log("Entering calculateOptimalVolume");
        ...
        ronsLogger.log(`calculateOptimalVolume: optimalVolume = ${optimalVolume}`);
...
async function checkBundleGas(bundleGas: BigNumber): Promise<boolean> {
    ...
    ronsLogger.log(`checkBundleGas: bundleGas = ${bundleGas}, isValid = ${isValid}`);
    return isValid;
}

export async function monitorCompetingBundlesGasPrices(blocksApi: { getRecentBlocks: () => any; }): Promise<Array<BigNumber>> {
    ronsLogger.log("Entering monitorCompetingBundlesGasPrices");
    ...
    ronsLogger.log(`monitorCompetingBundlesGasPrices: competingBundlesGasPrices = ${competingBundlesGasPrices}`);
