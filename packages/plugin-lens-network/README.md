# @rons-org/plugin-abstract

A plugin for interacting with the Abstract blockchain network within the RonsOS ecosystem.
...
```bash
pnpm install @rons-org/plugin-lensNetwork
```
...
```typescript
import { lensPlugin } from '@rons-org/plugin-lensNetwork';
```
