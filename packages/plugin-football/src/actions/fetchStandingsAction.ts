import type { HandlerCallback } from "@rons-org/core";
import {
    type Action,
    type IAgentRuntime,
    type Memory,
    ...
    handler: async (
        ...
        try {
            ...
            if (!apiKey?.match(/^[a-f0-9]{32}$/i)) {
                ronsLogger.error("Invalid API key format");
                return false;
            }

            const now = Date.now();
            if (now - lastRequestTime < RATE_LIMIT_WINDOW_MS) {
                ronsLogger.error("Rate limit exceeded");
                return false;
            }
            lastRequestTime = now;
            ...
            ronsLogger.error("Error in fetchStandingsAction:", error);
            return false;
        }
    },
    examples: [
