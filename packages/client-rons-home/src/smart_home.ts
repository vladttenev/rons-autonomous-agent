...
export class SmartHomeManager {
    ...
    async handleCommand(command: string, userId: string): Promise<any> {
        try {
            ...
            ronsLogger.error("Error handling smart home command:", error);
            throw error;
        }
    }
    ...
    private async getCurrentState(): Promise<string> {
        try {
            ...
            ronsLogger.error("Error getting current state:", error);
    ...
    private async executeCommand(deviceCommand: any): Promise<any> {
        try {
            ...
            ronsLogger.error("Error executing smart home command:", error);
            throw error;
        }
    }
}