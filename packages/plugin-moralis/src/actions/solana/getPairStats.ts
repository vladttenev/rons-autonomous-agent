...
export default {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting Moralis GET_SOLANA_PAIR_STATS handler...");
        ...
        try {
            ronsLogger.log("Composing pair stats context...");
            const statsContext = composeContext({
                state: currentState,
                template: getPairStatsTemplate,
            });

            ronsLogger.log("Extracting pair address...");
            ...
            ronsLogger.error("Error in GET_SOLANA_PAIR_STATS handler:", error);
            const errorMessage =
