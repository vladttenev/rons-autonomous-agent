...
const asteraiProvider: Provider = {
    get: async (
        ...
        if (!asteraiClient) {
            ronsLogger.error("asteraiClient is not initialised");
            return null;
        }
        ...
        if (!agentSummaryMemory) {
            ...
            ronsLogger.debug("asterai agent summary fetched:", summary);
        ...
        if (!agentSummaryMemory) {
            ronsLogger.error("failed to initialise agent's summary memory");
            return null;
        }
