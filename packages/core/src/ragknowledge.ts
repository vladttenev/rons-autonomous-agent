...
import ronsLogger from "./logger.ts";
import {
    type IAgentRuntime,
...
export class RAGKnowledgeManager implements IRAGKnowledgeManager {
    ...
    async createKnowledge(item: RAGKnowledgeItem): Promise<void> {
        if (!item.content.text) {
            ronsLogger.warn("Empty content in knowledge item");
            return;
        }
        ...
        try {
            ...
            ronsLogger.error(`Error processing knowledge ${item.id}:`, error);
            throw error;
        }
    }
    ...
    async cleanupDeletedKnowledgeFiles() {
        try {
            ...
            ronsLogger.debug("[Cleanup] Finished knowledge cleanup process");
        } catch (error) {
            ronsLogger.error(
    ...
    async processFile(file: {
        ...
        const timeMarker = (label: string) => {
            ...
            ronsLogger.info(`[Timing] ${label}: ${time.toFixed(2)}s`);
        };
        ...
        try {
            ...
            ronsLogger.info(`Generated ${totalChunks} chunks`);
            ...
            ronsLogger.error(`Error processing file ${file.path}:`, error);
            throw error;
        }
    }
}
