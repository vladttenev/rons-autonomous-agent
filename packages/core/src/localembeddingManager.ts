...
class LocalEmbeddingModelManager {
    ...
    private async getRootPath(): Promise<string> {
        ...
        return rootPath.includes("/rons/")
            ? rootPath.split("/rons/")[0] + "/rons/"
    ...
    private async initializeModel(): Promise<void> {
        ...
        try {
            ...
            ronsLogger.debug("Initializing BGE embedding model...");
            ...
            ronsLogger.debug("BGE model initialized successfully");
        } catch (error) {
            ronsLogger.error("Failed to initialize BGE model:", error);
            throw error;
        }
    }
    ...
    public async generateEmbedding(input: string): Promise<number[]> {
        ...
        try {
            ...
            // ronsLogger.debug("Raw embedding from BGE:", {
            ...
            ronsLogger.error("Embedding generation failed:", error);
            throw error;
        }
    }
