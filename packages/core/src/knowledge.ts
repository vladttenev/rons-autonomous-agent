...
async function get(
    ...
    if (!message?.content?.text) {
        ronsLogger.warn("Invalid message for knowledge query:", {
            message,
    ...
    ronsLogger.debug("Knowledge query:", {
    ...
    if (!processed || processed.trim().length === 0) {
        ronsLogger.warn("Empty processed text for knowledge query");
        return [];
    }
...
export function preprocess(content: string): string {
    ronsLogger.debug("Preprocessing text:", {
        input: content,
