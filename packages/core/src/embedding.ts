...
async function getRemoteEmbedding(
    ...
    try {
        ...
        if (!response.ok) {
            ronsLogger.error("API Response:", await response.text()); // Debug log
            throw new Error(
        ...
        ronsLogger.error("Full error details:", e);
        throw e;
    }
}
...
export async function embed(runtime: IAgentRuntime, input: string) {
    ronsLogger.debug("Embedding request:", {
    ...
    if (!input || typeof input !== "string" || input.trim().length === 0) {
        ronsLogger.warn("Invalid embedding input:", {
            input,
            type: typeof input,
            length: input?.length,
        });
    ...
    async function retrieveCachedEmbedding(
        ...
        if (!input) {
            ronsLogger.log("No input to retrieve cached embedding for");
            return null;
        }
