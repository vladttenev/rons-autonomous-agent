...
import { ronsLogger } from "./index.ts";
import {
    models,
    getModelSettings,
...
export async function trimTokens(
    ...
    ronsLogger.warn(`Unsupported tokenizer type: ${tokenizerType}`);
    return truncateTiktoken("gpt-4o", context, maxTokens);
}
...
async function truncateAuto(
    ...
    try {
        ...
        ronsLogger.error("Error in trimTokens:", error);
...
async function getOnChainEternalAISystemPrompt(
    ...
    if (agentId && providerUrl && contractAddress) {
        ...
        try {
            ...
            if (result) {
                ronsLogger.info("on-chain system-prompt response", result[0]);
                const value = result[0].toString().replace("0x", "");
                const content = Buffer.from(value, "hex").toString("utf-8");
                ronsLogger.info("on-chain system-prompt", content);
...
async function fetchEternalAISystemPrompt(
    ...
    if (containsSubstring) {
        ...
        ronsLogger.info("fetch lightHouse", lightHouse);
        const responseLH = await fetch(lightHouse, {
            method: "GET",
        });
        ronsLogger.info("fetch lightHouse resp", responseLH);
        if (responseLH.ok) {
            ...
            ronsLogger.info("fetch gcs", gcs);
            const responseGCS = await fetch(gcs, {
                method: "GET",
            });
            ronsLogger.info("fetch lightHouse gcs", responseGCS);
            if (responseGCS.ok) {
...
function getCloudflareGatewayBaseURL(
    ...
    ronsLogger.debug("Cloudflare Gateway Configuration:", {
    ...
    if (!isCloudflareEnabled) {
        ronsLogger.debug("Cloudflare Gateway is not enabled");
        return undefined;
    }
    ...
    ronsLogger.info("Using Cloudflare Gateway:", {
        provider,
        baseURL,
...
export async function generateText({
    ...
    ronsLogger.log("Generating text...");

    ronsLogger.info("Generating text with options:", {
        modelProvider: runtime.modelProvider,
        model: modelClass,
        verifiableInference,
    });
    ronsLogger.log("Using provider:", runtime.modelProvider);
    ...
    if (verifiableInference && runtime.verifiableInferenceAdapter) {
        ...
        try {
            ...
            ronsLogger.log("Verifiable inference result:", result);
            // Verify the proof
            const isValid =
            ...
            ronsLogger.error("Error in verifiable inference:", error);
            throw error;
        }
    }

    const provider = runtime.modelProvider;
    ronsLogger.debug("Provider settings:", {
        provider,
        hasRuntime: !!runtime,
        runtimeSettings: {
    ...
    ronsLogger.info("Selected model:", model);
    ...
    try {
        ...
        switch (provider) {
            ...
            case ModelProviderName.LMSTUDIO: {
                ...
                //ronsLogger.debug("OpenAI baseURL result:", { baseURL });
            ...
            case ModelProviderName.ETERNALAI: {
                ronsLogger.debug("Initializing EternalAI model.");
                const openai = createOpenAI({
                    apiKey,
                    baseURL: endpoint,
                    fetch: async (
                ...
                response = openaiResponse;
                ronsLogger.debug("Received response from EternalAI model.");
                break;
            }
            ...
            case ModelProviderName.GOOGLE: {
                ...
                ronsLogger.debug("Received response from Google model.");
                break;
            }
            ...
            case ModelProviderName.MISTRAL: {
                ...
                ronsLogger.debug("Received response from Mistral model.");
                break;
            }
            ...
            case ModelProviderName.ANTHROPIC: {
                ...
                ronsLogger.debug("Anthropic baseURL result:", { baseURL });
                ...
                ronsLogger.debug("Received response from Anthropic model.");
                break;
            }

            case ModelProviderName.CLAUDE_VERTEX: {
                ronsLogger.debug("Initializing Claude Vertex model.");
            ...
            case ModelProviderName.GROK: {
                ronsLogger.debug("Initializing Grok model.");
                ...
                ronsLogger.debug("Received response from Grok model.");
                break;
            }
            ...
            case ModelProviderName.GROQ: {
                ...
                ronsLogger.debug("Groq baseURL result:", { baseURL });
                ...
                ronsLogger.debug("Received response from Groq model.");
                break;
            }
            ...
            case ModelProviderName.LLAMALOCAL: {
                ...
                ronsLogger.debug("Received response from local Llama model.");
                break;
            }

            case ModelProviderName.REDPILL: {
                ronsLogger.debug("Initializing RedPill model.");
                ...
                ronsLogger.debug("Received response from redpill model.");
                break;
            }

            case ModelProviderName.OPENROUTER: {
                ronsLogger.debug("Initializing OpenRouter model.");
                const serverUrl = getEndpoint(provider);
                ...
                ronsLogger.debug("Received response from OpenRouter model.");
                break;
            }

            case ModelProviderName.OLLAMA:
                {
                    ronsLogger.debug("Initializing Ollama model.");
                ...
                ronsLogger.debug("Received response from Ollama model.");
                break;

            case ModelProviderName.HEURIST: {
                ronsLogger.debug("Initializing Heurist model.");
                ...
                ronsLogger.debug("Received response from Heurist model.");
                break;
            }
            case ModelProviderName.GAIANET: {
                ronsLogger.debug("Initializing GAIANET model.");
                ...
                ronsLogger.debug("Using GAIANET model with baseURL:", baseURL);
                ...
                response = openaiResponse;
                ronsLogger.debug("Received response from GAIANET model.");
                break;
            }

            case ModelProviderName.ATOMA: {
                ronsLogger.debug("Initializing Atoma model.");
                ...
                ronsLogger.debug("Received response from Atoma model.");
                break;
            }

            case ModelProviderName.GALADRIEL: {
                ronsLogger.debug("Initializing Galadriel model.");
                const headers = {};
                ...
                ronsLogger.debug("Received response from Galadriel model.");
                break;
            }

            case ModelProviderName.INFERA: {
                ronsLogger.debug("Initializing Infera model.");
                ...
                ronsLogger.debug("Received response from Infera model.");
                break;
            }

            case ModelProviderName.VENICE: {
                ronsLogger.debug("Initializing Venice model.");
                ...
                ronsLogger.debug("Received response from Venice model.");
                break;
            }

            case ModelProviderName.NVIDIA: {
                ronsLogger.debug("Initializing NVIDIA model.");
                ...
                ronsLogger.debug("Received response from NVIDIA model.");
                break;
            }

            case ModelProviderName.DEEPSEEK: {
                ronsLogger.debug("Initializing Deepseek model.");
                ...
                ronsLogger.debug("Received response from Deepseek model.");
                break;
            }

            case ModelProviderName.LIVEPEER: {
                ronsLogger.debug("Initializing Livepeer model.");
                ...
                const fetchResponse = await runtime.fetch(endpoint + "/llm", {
                    ...
                    headers: {
                        ...
                        "Content-Type": "application/json",
                        Authorization: "Bearer rons-app-llm",
                    },
        ...
        ronsLogger.error("Error in generateText:", error);
        throw error;
    }
}
...
export async function generateShouldRespond({
    ...
    while (true) {
        try {
            ...
            ronsLogger.debug("Received response from generateText:", response);
            const parsedResponse = parseShouldRespondFromText(response.trim());
            if (parsedResponse) {
                ronsLogger.debug("Parsed response:", parsedResponse);
                return parsedResponse;
            } else {
                ronsLogger.debug("generateShouldRespond no response");
            }
        } catch (error) {
            ronsLogger.error("Error in generateShouldRespond:", error);
            if (
        ...
        ronsLogger.log(`Retrying in ${retryDelay}ms...`);
...
export async function splitChunks(
    ...
    ronsLogger.debug(`[splitChunks] Starting text split`);
    ...
    ronsLogger.debug(`[splitChunks] Split complete:`, {
...
export async function generateTrueOrFalse({
    ...
    while (true) {
        try {
            ...
            ronsLogger.error("Error in generateTrueOrFalse:", error);
        }
...
export async function generateTextArray({
    ...
    if (!context) {
        ronsLogger.error("generateTextArray context is empty");
        return [];
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            ronsLogger.error("Error in generateTextArray:", error);
        }
...
export async function generateObjectDeprecated({
    ...
    if (!context) {
        ronsLogger.error("generateObjectDeprecated context is empty");
        return null;
    }
    let retryDelay = 1000;
    ...
    while (true) {
        try {
            ...
            ronsLogger.error("Error in generateObject:", error);
        }
...
export async function generateObjectArray({
    ...
    if (!context) {
        ronsLogger.error("generateObjectArray context is empty");
        return [];
    }
    let retryDelay = 1000;
...
export async function generateMessageResponse({
    ...
    context = await trimTokens(context, max_context_length, runtime);
    ronsLogger.debug("Context:", context);
    let retryLength = 1000; // exponential backoff
    while (true) {
        try {
            ronsLogger.log("Generating message response..");
            ...
            if (!parsedContent) {
                ronsLogger.debug("parsedContent is null, retrying");
                continue;
            }

            return parsedContent;
        } catch (error) {
            ronsLogger.error("ERROR:", error);
            // wait for 2 seconds
            retryLength *= 2;
            await new Promise((resolve) => setTimeout(resolve, retryLength));
            ronsLogger.debug("Retrying...");
        }
    }
}
...
export const generateImage = async (
    ...
    if (!modelSettings) {
        ronsLogger.warn("No model settings found for the image model provider.");
        return { success: false, error: "No model settings available" };
    }
    const model = modelSettings.name;
    ronsLogger.info("Generating image with options:", {
        ...
        if (runtime.imageModelProvider === ModelProviderName.HEURIST) {
            ...
            const base64s = await Promise.all(
                togetherResponse.data.map(async (image) => {
                    if (!image.url) {
                        ronsLogger.error("Missing URL in image data:", image);
            ...
            ronsLogger.debug(`Generated ${base64s.length} images`);
            ...
            const result = await fal.subscribe(model, {
                ...
                onQueueUpdate: (update) => {
                    if (update.status === "IN_PROGRESS") {
                        ronsLogger.info(update.logs.map((log) => log.message));
                    }
                },
            });
            ...
            try {
                ...
                const response = await fetch(
                    `${baseUrl.toString()}text-to-image`,
                    {
                        ...
                        headers: {
                            "Content-Type": "application/json",
                            Authorization: "Bearer rons-app-img",
                        },
                        body: JSON.stringify({
                            model_id:
...
async function handleAnthropic({
    ...
    ronsLogger.debug("Handling Anthropic request with Cloudflare check");
    if (mode === "json") {
        ronsLogger.warn("Anthropic mode is set to json, changing to auto");
        mode = "auto";
    }
    const baseURL = getCloudflareGatewayBaseURL(runtime, "anthropic");
    ronsLogger.debug("Anthropic handleAnthropic baseURL:", { baseURL });
...
async function handleGroq({
    ...
    ronsLogger.debug("Handling Groq request with Cloudflare check");
    const baseURL = getCloudflareGatewayBaseURL(runtime, "groq");
    ronsLogger.debug("Groq handleGroq baseURL:", { baseURL });
...
export async function generateTweetActions({
    ...
    while (true) {
        try {
            ...
            if (actions) {
                ronsLogger.debug("Parsed tweet actions:", actions);
                return actions;
            } else {
                ronsLogger.debug("generateTweetActions no valid response");
            }
        } catch (error) {
            ronsLogger.error("Error in generateTweetActions:", error);
            if (
                error instanceof TypeError &&
                error.message.includes("queueTextCompletion")
            ) {
