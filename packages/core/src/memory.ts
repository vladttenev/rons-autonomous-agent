...
export class MemoryManager implements IMemoryManager {
    ...
    async addEmbeddingToMemory(memory: Memory): Promise<Memory> {
        ...
        try {
            ...
            ronsLogger.error("Failed to generate embedding:", error);
    ...
    async createMemory(memory: Memory, unique = false): Promise<void> {
        ...
        if (existingMessage) {
            ronsLogger.debug("Memory already exists, skipping");
            return;
        }

        ronsLogger.log("Creating Memory", memory.id, memory.content.text);
