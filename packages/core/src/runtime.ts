...
export class AgentRuntime implements IAgentRuntime {
    ...
    getService<T extends Service>(service: ServiceType): T | null {
        ...
        if (!serviceInstance) {
            ronsLogger.error(`Service ${service} not found`);
            return null;
        }
    ...
    async registerService(service: Service): Promise<void> {
        ...
        ronsLogger.log(`${this.character.name}(${this.agentId}) - Registering service:`, serviceType);
        ...
        ronsLogger.success(`${this.character.name}(${this.agentId}) - Service ${serviceType} registered successfully`);
    }
    ...
    constructor(opts: {
        ...
        ronsLogger.info(`${this.character.name}(${this.agentId}) - Initializing AgentRuntime with options:`, {
        ...
        ronsLogger.success(`Agent ID: ${this.agentId}`);
        ...
        ronsLogger.info(`${this.character.name}(${this.agentId}) - Setting Model Provider:`, {
        ...
        if (!Object.values(ModelProviderName).includes(this.modelProvider)) {
            ronsLogger.error("Invalid model provider:", this.modelProvider);
            ronsLogger.error(
                "Available providers:",
        ...
        if (!this.serverUrl) {
            ronsLogger.warn("No serverUrl provided, defaulting to localhost");
        }
    ...
    async initialize() {
        ...
        */
        ...
        if (
            ...
            ronsLogger.info(`[RAG Cleanup] Cleanup complete`);
        }
    }

    async stop() {
        ronsLogger.debug("runtime::stop - character", this.character.name);
            ...
            try {
                ...
                if (
                    ...
                    try {
                        ...
                        ronsLogger.debug("[RAG Query Result]", {
                            relativePath: contentItem,
                            fullPath: filePath,
                            knowledgeId,
                            isShared,
    ...
    private async processCharacterRAGDirectory(dirConfig: {
        ...
        if (!dirConfig.directory) {
            ronsLogger.error("[RAG Directory] No directory specified");
            return;
        }
        ...
        try {
            ...
            ronsLogger.debug(`[RAG Directory] Searching in: ${dirPath}`);
    ...
    registerAction(action: Action) {
        ronsLogger.success(`${this.character.name}(${this.agentId}) - Registering action: ${action.name}`);
    ...
    async processActions(
        ...
        for (const response of responses) {
            if (!response.content?.action) {
                ronsLogger.warn("No action found in the response content.");
                continue;
            }
            ...
            ronsLogger.success(`Normalized action: ${normalizedAction}`);
            ...
            if (!action) {
                ronsLogger.info("Attempting to find action in similes.");
            ...
            if (!action.handler) {
                ronsLogger.error(`Action ${action.name} has no handler.`);
                continue;
            }
    ...
    async evaluate(
        ...
        const evaluatorPromises = this.evaluators.map(
            async (evaluator: Evaluator) => {
                ronsLogger.log("Evaluating", evaluator.name);
    ...
    async ensureUserExists(
        ...
        if (!account) {
            ...
            ronsLogger.success(`User ${userName} created successfully.`);
        }
    }
    ...
    async ensureRoomExists(roomId: UUID) {
        ...
        if (!room) {
            ...
            ronsLogger.log(`Room ${roomId} created successfully.`);
        }
    }
