...
export class DevaController {
    ...
    private async populatePersona() {
        ...
        if (!this.persona || !this.persona.id) {
            ronsLogger.error("❌ Deva Client failed to fetch Persona");
    ...
    private async populatePosts() {
        ...
        for (const post of notExistingPostsInMemory) {
            ronsLogger.log("Saving Post", post.id);
            ...
            ronsLogger.log("Creating memory for post", post.id);
            ...
            ronsLogger.log("Created memory for post", post.id);
        }
    ...
    private async setupPostAwaiter() {
        ...
        ronsLogger.log(`Next post scheduled in ${randomMinutes} minutes`);
    }

    private async generateNewPost() {
        ronsLogger.log("Generating new Deva Post");
        ...
        ronsLogger.log(`New Post published:\n ${newPostContent}`);
    }
}
