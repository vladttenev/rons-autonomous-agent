...
export async function generateSQL(prompt: string): Promise<string> {
    try {
        ...
        ronsLogger.log("Generated SQL:", data.sql);
        return data.sql;
    } catch (error) {
        ronsLogger.error("Error generating SQL:", error);
        throw error;
    }
}
...
export async function executeQuery(sql: string): Promise<{
    ...
    try {
        ...
        // 1. Execute query
        ronsLogger.log("Executing Chainbase query:", processedSql);
        ...
        ronsLogger.log("Execute response:", executeData);
        ...
        while (retries < MAX_RETRIES) {
            ...
            ronsLogger.log("Poll response:", response);
            ...
            if (response.data.status === "FINISHED") {
                ronsLogger.log("Query succeeded:", response.data);
                return {
        ...
        ronsLogger.error("Error executing Chainbase query:", error);
        throw error;
    }
}
...
export async function getTokenBalances(
    ...
    try {
        ...
        ronsLogger.log("Fetching token balances:", params);
        ...
        ronsLogger.log("Token balances retrieved:", data);
        ...
        ronsLogger.error("Error fetching token balances:", error);
        throw error;
    }
}
