...
Sei Network plugin for Rons OS that enables Rons agents to perform actions on the Sei blockchain.
## Overview
...
```bash
pnpm install @rons-org/plugin-sei
```
...
```typescript
import { seiPlugin } from "@rons-org/plugin-sei";
