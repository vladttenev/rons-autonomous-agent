# @rons-org/plugin-flow

A plugin for interacting with the Flow blockchain within the RonsOS ecosystem.
...
```bash
pnpm install @rons-org/plugin-flow
```
...
```typescript
import { flowPlugin } from "@rons-org/plugin-flow";
```
