...
export class FlowWalletProvider implements IFlowSigner, IFlowScriptExecutor {
    ...
    async syncAccountInfo() {
        ...
        ronsLogger.debug("Flow account info synced", {
...
function getSignerAddress(runtime: IAgentRuntime): string {
    ...
    if (!signerAddr) {
        ronsLogger.error("No signer address");
    ...
    get: async (
        ...
        try {
            ...
            if (!info || info?.address !== walletProvider.address) {
                ronsLogger.error("Invalid account info");
                return null;
            }
            ...
            ronsLogger.error("Error in Flow wallet provider:", error.message);
            return null;
        }
    },
};
