import type { AwsS3Service } from "@rons-org/plugin-node";
import {
    composeContext,
    ronsLogger,
    generateImage,
...
import {
    saveBase64Image,
    saveHeuristImage,
} from "@rons-org/plugin-image-generation";
...
export async function createNFTMetadata({
    ...
    ronsLogger.log("NFT Prompt:", nftPrompt);
    if (images.success && images.data && images.data.length > 0) {
        const image = images.data[0];
        const filename = `${tokenId}`;
        if (image.startsWith("http")) {
            ronsLogger.log("Generating image url:", image);
        }
...
export async function createNFT({
    ...
    if (nftInfo) {
        ...
        ronsLogger.log("NFT ID:", nftAddressRes.address);
        return {
            network: "solana",
