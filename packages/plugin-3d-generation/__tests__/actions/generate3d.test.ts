...
import type { Memory, State, IAgentRuntime, HandlerCallback } from '@rons-org/core';
...
describe('ThreeDGeneration Action', () => {
    ...
    describe('validate', () => {
        it('should validate successfully with API key', async () => {
            ...
            expect(ronsLogger.log).toHaveBeenCalledWith('FAL_API_KEY present:', true);
        });
        ...
        it('should fail validation without API key', async () => {
            ...
            expect(ronsLogger.log).toHaveBeenCalledWith('FAL_API_KEY present:', false);
        });
    });
