...
export class WalletProvider {
    ...
    async fetchPortfolio(): Promise<WalletPortfolio> {
        try {
            ...
            if (cachedValue) {
                ronsLogger.info("Cache hit for fetchPortfolio");
                return cachedValue;
            }
            ronsLogger.info("Cache miss for fetchPortfolio");
            ...
            if (!portfolioData || !portfolioData.success) {
                ronsLogger.error("Failed to fetch portfolio:", portfolioData);
                throw new Error(
    ...
    async fetchAllTokens(): Promise<Array<Item>> {
        try {
            ...
            if (cachedValue) {
                ronsLogger.log("Cache hit for fetch all");
                return cachedValue;
            }
            ronsLogger.log("Cache miss for fetch all");
            ...
            if (!tokensData || tokensData.error || !tokensData.data) {
                ronsLogger.error("Failed to fetch all tokens:", tokensData);
                throw new Error(
            ...
            ronsLogger.error("Error fetching all tokens:", error);
            throw error;
        }
    }
}
