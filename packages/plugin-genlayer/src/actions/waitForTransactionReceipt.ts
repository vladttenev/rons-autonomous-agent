...
export const waitForTransactionReceiptAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting wait for transaction receipt action");
        ...
        ronsLogger.info(`Waiting for receipt of transaction: ${hashMatch[0]}`);
        const result = await clientProvider.client.waitForTransactionReceipt({
            hash: hashMatch[0] as TransactionHash,
        });

        ronsLogger.success("Successfully received transaction receipt");
        ronsLogger.debug("Transaction receipt:", result);
        await callback(
            {
