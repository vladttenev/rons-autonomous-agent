...
import type { WriteContractParams } from "../types";
...
export const writeContractAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting write contract action");
        ...
        if (!options) {
            ronsLogger.error("Failed to parse write contract parameters");
            throw new Error("Failed to parse write contract parameters");
        }

        ronsLogger.debug("Parsed parameters:", options);
        ronsLogger.info(
