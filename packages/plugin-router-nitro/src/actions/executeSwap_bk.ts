...
export const executeSwapAction = {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting ROUTER_NITRO_SWAP handler...");
        ...
        ronsLogger.log("swap content: ", JSON.stringify(content));
        ...
        try {
            ...
            ronsLogger.log(`Chain Data Details: ${JSON.stringify(swapDetails)}`);
            ...
            if (!swapDetails.fromChainId || !swapDetails.toChainId) {
                ronsLogger.log("Invalid chain data details");
                return false;
            }
            else {
           ...
           if (BigInt(userBalance) < amountIn) {
                        ronsLogger.log("Insufficient balance to perform the swap");
       ...
       const pathfinderResponse = await fetchPathfinderQuote(pathfinderParams) as PathfinderResponse;
                if (pathfinderResponse) {
                    ...
                    ronsLogger.log(`Quote: ${normalizedAmountOut}`);
                    ...
                    try {
                        ...
                        if (blockExplorerUrl) {
                            ...
                            ronsLogger.log(`Transaction Explorer URL: ${txExplorerUrl}`);
                            callback?.({
            ...
            ronsLogger.log(`Error during executing swap: ${error.message}`);
