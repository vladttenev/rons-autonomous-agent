...
export const webhookProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.debug("Starting webhookProvider.get function");
        try {
            Coinbase.configure({
                apiKeyName:
            ...
            ronsLogger.info("Listing all webhooks:", resp.data);
            ...
            ronsLogger.error("Error in webhookProvider:", error);
            return [];
        }
    },
};
...
export const createWebhookAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.info("Validating runtime for CREATE_WEBHOOK...");
        return (
            !!(
    ...
    handler: async (
        ...
        ronsLogger.debug("Starting CREATE_WEBHOOK handler...");
        ...
        try {
            ...
            ronsLogger.info("Creating webhook with details:", {
                networkId,
                notificationUri,
                eventType,
                eventTypeFilter,
            ...
            ronsLogger.info("Webhook appended to CSV successfully");
        } catch (error) {
            ronsLogger.error("Error during webhook creation:", error);
            callback(
                {
