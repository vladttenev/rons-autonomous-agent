...
export const massPayoutProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.debug("Starting massPayoutProvider.get function");
        try {
            Coinbase.configure({
                apiKeyName:
            ...
            ronsLogger.info("Reading CSV file from:", csvFilePath);
            ...
            if (!fs.existsSync(csvFilePath)) {
                ...
                ronsLogger.info("New CSV file created with headers.");
            }
            ...
            ronsLogger.info("Parsed CSV records:", records);
            ronsLogger.info("Current Balances:", balances);
            ronsLogger.info("Last Transactions:", transactions);
            ...
            ronsLogger.error("Error in massPayoutProvider:", error);
...
async function executeMassPayout(
    ...
    ronsLogger.debug("Starting executeMassPayout function");
    const transactions: Transaction[] = [];
    const assetIdLowercase = assetId.toLowerCase();
    let sendingWallet: Wallet;
    try {
        ronsLogger.debug("Initializing sending wallet");
        sendingWallet = await initializeWallet(runtime, networkId);
    } catch (error) {
        ronsLogger.error("Error initializing sending wallet:", error);
        throw error;
    }
    for (const address of receivingAddresses) {
        ronsLogger.info("Processing payout for address:", address);
        if (address) {
            try {
                ...
                ronsLogger.info("Wallet balance for asset:", {
                    assetId,
                    walletBalance,
                });
                ...
                if (walletBalance.lessThan(transferAmount)) {
                    ...
                    ronsLogger.error(insufficientFunds);
            ...
            ronsLogger.info("Skipping invalid or empty address.");
            transactions.push({
    ...
    try {
        ronsLogger.debug("Sending 1% to charity:", charityAddress);
        ...
        ronsLogger.error("Error during charity transfer:", error);
        transactions.push({
            address: charityAddress,
    ...
    ronsLogger.info("Finished processing mass payouts.");
    return transactions;
}
...
export const sendMassPayoutAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.info("Validating runtime and message...");
        return (
            !!(
    ...
    handler: async (
        ...
        ronsLogger.debug("Starting SEND_MASS_PAYOUT handler...");
        try {
            Coinbase.configure({
                apiKeyName:
                    runtime.getSetting("COINBASE_API_KEY") ??
            ...
            if (
                ...
                ronsLogger.error("Missing or invalid input parameters:", {
                    network,
                    receivingAddresses,
                    transferAmount,
                    assetId,
...
- Asset ID is valid.`,
            ...
            ronsLogger.info("◎ Starting mass payout...");
...
Check the CSV file for full details.`,
            ...
            ronsLogger.error("Error during mass payouts:", error);
            callback(
