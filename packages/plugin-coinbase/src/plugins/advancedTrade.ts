...
const tradeProvider: Provider = {
    get: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.debug("Starting tradeProvider function");
        try {
            ...
            try {
                ...
                ronsLogger.error("Error fetching accounts:", error);
                return [];
            }
            ...
            try {
                ...
                ronsLogger.error("Error fetching products:", error);
                return [];
            }
            ...
            try {
                ...
                ronsLogger.error("Error reading CSV file:", error);
                return [];
            }
            ...
            try {
                ...
                ronsLogger.error("Error parsing CSV data:", error);
                return [];
            }
            ...
            ronsLogger.error("Error in tradeProvider:", error);
            return [];
        }
    },
};
...
export async function appendTradeToCsv(tradeResult: any) {
    ronsLogger.debug("Starting appendTradeToCsv function");
    try {
        const csvWriter = createArrayCsvWriter({
            path: tradeCsvFilePath,
        ...
        ronsLogger.info("Trade result:", tradeResult);
        ...
        ronsLogger.info("Formatted trade for CSV:", formattedTrade);
        await csvWriter.writeRecords([formattedTrade]);
        ronsLogger.info("Trade written to CSV successfully");
    } catch (error) {
        ronsLogger.error("Error writing trade to CSV:", error);
        // Log the actual error for debugging
        if (error instanceof Error) {
            ronsLogger.error("Error details:", error.message);
        }
    }
}
...
async function hasEnoughBalance(
    ...
    ronsLogger.debug("Starting hasEnoughBalance function");
    try {
        const response = await client.listAccounts({});
        const accounts = JSON.parse(response);
        ronsLogger.info("Accounts:", accounts);
        ...
        if (!account) {
            ronsLogger.error(`No ${checkCurrency} account found`);
            return false;
        }
        ...
        ronsLogger.info(`Has sufficient balance: ${hasBalance}`);

        return hasBalance;
    } catch (error) {
        ronsLogger.error("Balance check failed with error:", {
...
export const executeAdvancedTradeAction: Action = {
    ...
    handler: async (
        ...
        // Initialize client
        ronsLogger.debug("Starting advanced trade client initialization");
        try {
            ...
            ronsLogger.info("Advanced trade client initialized");
        } catch (error) {
            ronsLogger.error("Client initialization failed:", error);
            callback(
                {
        ...
        let tradeDetails;
        ronsLogger.debug("Starting trade details generation");
        try {
            ...
            ronsLogger.info("Trade details generated:", tradeDetails.object);
        } catch (error) {
            ronsLogger.error("Trade details generation failed:", error);
            callback(
                {
        ...
        if (!isAdvancedTradeContent(tradeDetails.object)) {
            ronsLogger.error("Invalid trade content:", tradeDetails.object);
            callback(
                {
        ...
        ronsLogger.debug("Starting order configuration");
        try {
            if (orderType === "MARKET") {
                orderConfiguration =
                    side === "BUY"
            ...
            ronsLogger.error("Order configuration failed:", error);
            callback(
                {
                    text:
        ...
        try {
            ronsLogger.debug("Executing the trade");
            if (
            ...
            ronsLogger.info("Trade executed successfully:", order);
        } catch (error) {
            ronsLogger.error("Trade execution failed:", error?.message);
            callback(
                {
        ...
        try {
            ...
            ronsLogger.info("Trade logged to CSV");
        } catch (csvError) {
            ronsLogger.warn("Failed to log trade to CSV:", csvError);
