# @rons-org/plugin-coinbase

A comprehensive Coinbase integration plugin for RonsOS that provides access to Coinbase's various APIs and services.
...
```bash
npm install @rons-org/plugin-coinbase
```
...
```typescript
import { plugins } from "@rons-org/plugin-coinbase";
...
- `@rons-org/core`: Core RonsOS functionality
