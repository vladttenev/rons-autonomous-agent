...
```bash
npm install @rons-org/plugin-rabbi-trader
```
...
```typescript
import createRabbiTraderPlugin from '@rons-org/plugin-rabbi-trader';
...
- `@rons-org/core`: Core agent runtime
- `@rons-org/plugin-solana`: Solana integration
- `@rons-org/plugin-trustdb`: Trust score database
