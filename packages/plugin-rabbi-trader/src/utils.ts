import { ronsLogger, type IAgentRuntime, settings, type State } from "@rons-org/core";
...
export async function fetchWithRetry(
    ...
    for (let i = 0; i < PROVIDER_CONFIG.MAX_RETRIES; i++) {
        try {
            ronsLogger.log(`Attempt ${i + 1} for ${url} with chain ${chain}`);
            ...
            ronsLogger.error(`Attempt ${i + 1} failed:`, {
    ...
    try {
        ...
        if (state[historyKey]) {
            try {
                ...
                ronsLogger.warn("Failed to parse history, resetting", e);
            }
        }
        ...
        ronsLogger.error("Failed to manage analyzed tokens history:", {
