# @rons-org/plugin-fuel

A plugin for interacting with the Fuel blockchain within the RonsOS ecosystem.
...
```bash
pnpm install @rons-org/plugin-fuel
```
...
```typescript
import { fuelPlugin } from "@rons-org/plugin-fuel";
```
