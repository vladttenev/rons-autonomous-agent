# @rons-org/plugin-quai

Quai Network integration plugin for Rons OS that enables native token transfers and interactions with the Quai blockchain.
...
```bash
pnpm install @rons-org/plugin-quai
```
...
```typescript
import { quaiPlugin } from '@rons-org/plugin-quai';
