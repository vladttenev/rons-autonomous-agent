...
export class VideoService extends Service implements IVideoService {
    ...
    public async downloadMedia(url: string): Promise<string> {
        ...
        try {
            ...
            ronsLogger.log("Error downloading media:", error);
    ...
    public async downloadVideo(videoInfo: any): Promise<string> {
        ...
        try {
            ...
            ronsLogger.log("Error downloading video:", error);
    ...
    private async processVideoFromUrl(
        ...
        if (cached) {
            ronsLogger.log("Returning cached video file");
            return cached;
        }

        ronsLogger.log("Cache miss, processing video");
        ronsLogger.log("Fetching video info");
        const videoInfo = await this.fetchVideoInfo(url);
        ronsLogger.log("Getting transcript");
    ...
    async fetchVideoInfo(url: string): Promise<any> {
        if (url.endsWith(".mp4") || url.includes(".mp4?")) {
            try {
                ...
                ronsLogger.log("Error downloading MP4 file:", error);
        ...
        try {
            ...
            ronsLogger.log("Error fetching video info:", error);
    ...
    private async getTranscript(
        ...
        try {
            ...
            if (videoInfo.subtitles && videoInfo.subtitles.en) {
                ronsLogger.log("Manual subtitles found");
            ...
            if (
                ...
                ronsLogger.log("Automatic captions found");
            ...
            if (
                ...
                ronsLogger.log("Music video detected, no lyrics available");
            ...
            ronsLogger.log("Error in getTranscript:", error);
            throw error;
        }
    }

    private async downloadCaption(url: string): Promise<string> {
        ronsLogger.log("Downloading caption from:", url);
        const response = await fetch(url);
        if (!response.ok) {
            throw new Error(
    ...
    private parseCaption(captionContent: string): string {
        ronsLogger.log("Parsing caption");
        try {
            ...
            if (jsonContent.events) {
                ...
                ronsLogger.log("Unexpected caption format:", jsonContent);
                return "Error: Unable to parse captions";
            }
        } catch (error) {
            ronsLogger.log("Error parsing caption:", error);
            return "Error: Unable to parse captions";
        }
    }
    ...
    async transcribeAudio(
        ...
        ronsLogger.log("Preparing audio for transcription...");
        ...
        if (!fs.existsSync(mp3FilePath)) {
            if (fs.existsSync(mp4FilePath)) {
                ronsLogger.log("MP4 file found. Converting to MP3...");
                await this.convertMp4ToMp3(mp4FilePath, mp3FilePath);
            } else {
                ronsLogger.log("Downloading audio...");
                await this.downloadAudio(url, mp3FilePath);
            }
        }

        ronsLogger.log(`Audio prepared at ${mp3FilePath}`);

        const audioBuffer = fs.readFileSync(mp3FilePath);
        ronsLogger.log(`Audio file size: ${audioBuffer.length} bytes`);

        ronsLogger.log("Starting transcription...");
    ...
    private async convertMp4ToMp3(
        ...
        return new Promise((resolve, reject) => {
            ffmpeg(inputPath)
                ...
                .on("end", () => {
                    ronsLogger.log("Conversion to MP3 complete");
                    resolve();
                })
                .on("error", (err) => {
                    ronsLogger.log("Error converting to MP3:", err);
                    reject(err);
                })
                .run();
        });
    ...
    private async downloadAudio(
        ...
        try {
            ...
            ronsLogger.log("Error downloading audio:", error);
