...
class LocalImageProvider implements ImageProvider {
    ...
    async initialize(): Promise<void> {
        ...
        ronsLogger.info("Downloading Florence model...");
        ...
        ronsLogger.info("Downloading processor...");
        this.processor = (await AutoProcessor.from_pretrained(
            this.modelId
        )) as Florence2Processor;

        ronsLogger.info("Downloading tokenizer...");
        this.tokenizer = await AutoTokenizer.from_pretrained(this.modelId);
        ronsLogger.success("Image service initialization complete");
    }
    ...
    async describeImage(
        ...
        ronsLogger.log("Generating image description");
    ...
    implements IImageDescriptionService
{
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ronsLogger.log("Initializing ImageDescriptionService");
    ...
    private async initializeProvider(): Promise<boolean> {
        ...
        if (this.runtime.imageVisionModelProvider) {
            if (
                ...
                ronsLogger.debug("Using local provider for vision model");
            } else if (
                this.runtime.imageVisionModelProvider ===
                ModelProviderName.ANTHROPIC
            ) {
                this.provider = new AnthropicImageProvider(this.runtime);
                ronsLogger.debug("Using anthropic for vision model");
            } else if (
                this.runtime.imageVisionModelProvider ===
                ModelProviderName.GOOGLE
            ) {
                this.provider = new GoogleImageProvider(this.runtime);
                ronsLogger.debug("Using google for vision model");
            } else if (
                this.runtime.imageVisionModelProvider ===
                ModelProviderName.OPENAI
            ) {
                this.provider = new OpenAIImageProvider(this.runtime);
                ronsLogger.debug("Using openai for vision model");
            } else if (
                this.runtime.imageVisionModelProvider === ModelProviderName.GROQ
            ) {
                this.provider = new GroqImageProvider(this.runtime);
                ronsLogger.debug("Using Groq for vision model");
            } else {
                ronsLogger.warn(
            ...
            ronsLogger.debug("Using default openai for vision model");
            this.provider = new OpenAIImageProvider(this.runtime);
        }
    ...
    async describeImage(
        ...
        if (this.initialized) {
            try {
                ...
                ronsLogger.error("Error in describeImage:", error);
                throw error;
            }
        }
    }
