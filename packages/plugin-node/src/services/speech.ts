...
import { type IAgentRuntime, type ISpeechService, ServiceType } from "@rons-org/core";
...
async function getVoiceSettings(runtime: IAgentRuntime) {
    ...
    ronsLogger.debug("Voice settings:", {
        hasElevenLabs,
        useVits,
        voiceSettings,
        elevenlabsSettings,
    ...
    try {
        ...
        if (error.message === "QUOTA_EXCEEDED") {
            ...
            if (audio instanceof Buffer) {
                ronsLogger.log("audio is a buffer");
                wavStream = Readable.from(audio);
            } else if ("audioChannels" in audio && "sampleRate" in audio) {
                ronsLogger.log("audio is a RawAudio");
                const floatBuffer = Buffer.from(audio.audioChannels[0].buffer);
                ronsLogger.log("buffer length: ", floatBuffer.length);
