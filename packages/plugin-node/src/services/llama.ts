...
import { Service } from "@rons-org/core";
import fs from "fs";
...
export class LlamaService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ronsLogger.info("Initializing LlamaService...");
    ...
    private async ensureInitialized() {
        if (!this.modelInitialized) {
            ...
            ronsLogger.info("Model already initialized");
        }
    }

    async initializeModel() {
        try {
            ronsLogger.info("Checking model file...");
            ...
            ronsLogger.info("Initializing Llama instance...");
            this.llama = await getLlama({
                gpu: hasCUDA ? "cuda" : undefined,
            });

            ronsLogger.info("Creating JSON schema grammar...");
            ...
            ronsLogger.info("Loading model...");
            this.model = await this.llama.loadModel({
                modelPath: this.modelPath,
            });

            ronsLogger.info("Creating context and sequence...");
            this.ctx = await this.model.createContext({ contextSize: 8192 });
            this.sequence = this.ctx.getSequence();

            this.modelInitialized = true;
            ronsLogger.success("Model initialization complete");
            this.processQueue();
        } catch (error) {
            ronsLogger.error(
    ...
    async checkModel() {
        if (!fs.existsSync(this.modelPath)) {
            ronsLogger.info("Model file not found, starting download...");
                    ...
                    https
                        .get(url, (response) => {
                            ...
                            response.on("data", (chunk) => {
                                ...
                                ronsLogger.progress(progressString);
                            });

                            file.on("finish", () => {
                                file.close();
                                ronsLogger.progress(""); // Clear the progress line
                                ronsLogger.success("Model download complete");
                                resolve();
                            });
            ...
            ronsLogger.warn("Model already exists.");
        }
    }
    ...
    async completion(prompt: string, runtime: IAgentRuntime): Promise<string> {
        try {
            ...
            ronsLogger.error("Error in completion:", error);
            throw error;
        }
    }
    ...
    async embedding(text: string, runtime: IAgentRuntime): Promise<number[]> {
        try {
            ...
            ronsLogger.error("Error in embedding:", error);
            throw error;
        }
    }
    ...
    private async getCompletionResponse(
        ...
        if (useGrammar) {
            ...
            try {
                ...
                ronsLogger.error("Error parsing JSON:", error);
            }
        } else {
            await this.sequence.clearHistory();
            return response;
    ...
    private async localCompletion(prompt: string): Promise<string> {
        ...
        for await (const token of this.sequence.evaluate(tokens, {
            ...
            if (current.includes("\n")) {
                ronsLogger.info("Stop sequence found");
                break;
            }
            ...
            if (responseTokens.length > 256) {
                ronsLogger.info("Max tokens reached");
                break;
            }
        }
