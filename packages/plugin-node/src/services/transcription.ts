...
import { Service, ServiceType } from "@rons-org/core";
    ...
    implements ITranscriptionService
{
    ...
    private async convertAudio(inputBuffer: ArrayBuffer): Promise<Buffer> {
        ...
        try {
            ...
            ronsLogger.log("Input audio info:", stream);
            ...
            ronsLogger.log("FFmpeg command:", ffmpegCommand);
            ...
            ronsLogger.error("Error converting audio:", error);
            throw error;
        }
    }
    ...
    private async saveDebugAudio(audioBuffer: ArrayBuffer, prefix: string) {
        ...
        ronsLogger.log(`Debug audio saved: ${filePath}`);
    }
    ...
    private async transcribeWithOpenAI(
        ...
        ronsLogger.log("Transcribing audio with OpenAI...");
        ...
        try {
            ...
            ronsLogger.log(`OpenAI speech to text result: "${trimmedResult}"`);
            ...
            if (error.response) {
                ronsLogger.error("Response data:", error.response.data);
                ronsLogger.error("Response status:", error.response.status);
                ronsLogger.error("Response headers:", error.response.headers);
            } else if (error.request) {
                ronsLogger.error("No response received:", error.request);
            } else {
                ronsLogger.error("Error setting up request:", error.message);
            }
            return null;
        }
    }
    ...
    public async transcribeLocally(
        ...
        try {
            ronsLogger.log("Transcribing audio locally...");
            ...
            ronsLogger.debug(`Temporary WAV file created: ${tempWavFile}`);
            ...
            if (!output || output.length < 5) {
                ronsLogger.log("Output is null or too short, returning null");
                return null;
            }
            return output;
        } catch (error) {
