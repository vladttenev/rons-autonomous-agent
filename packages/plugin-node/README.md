# @rons-org/plugin-node

Core Node.js plugin for Rons OS that provides essential services and actions for file operations, media processing, and cloud integrations.

## Overview

The Node plugin serves as a foundational component of Rons OS, bridging core Node.js capabilities with the Rons ecosystem. It provides crucial services for file operations, media processing, speech synthesis, and cloud integrations, enabling both local and cloud-based functionality for Rons agents.
...
```bash
npm install @rons-org/plugin-node
```
...
// Register with Rons OS
rons-org.registerPlugin(nodePlugin);
```
...
```typescript
process.env.DEBUG = "rons:plugin-node:*";
```
