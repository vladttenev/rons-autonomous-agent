...
A plugin enabling integration with the OriginTrail Decentralized Knowledge Graph (DKG) for enhanced search and knowledge management capabilities in RonsOS agents.

## Description

The DKG plugin extends RonsOS functionality by allowing agents to interact with the OriginTrail Decentralized Knowledge Graph. This plugin enables SPARQL-based searches on the DKG and combines these results with Rons's regular search results. Additionally, it creates a memory as a Knowledge Asset on the DKG after a response, making it available for future SPARQL queries.

## Installation

```bash
pnpm install @rons-org/plugin-dkg
```
...
- Combine DKG query results with Rons's internal search capabilities.
...
- Integrates DKG data with Rons’s response system.
