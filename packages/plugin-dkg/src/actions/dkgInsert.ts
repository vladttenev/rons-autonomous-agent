...
export const dkgInsert: Action = {
    ...
    handler: async (
        ...
        if (match?.[1]) {
            twitterUser = match[1];
            ronsLogger.log(`Extracted user: @${twitterUser}`);
        } else {
            ronsLogger.error("No user mention found or invalid input.");
        }
        ...
        if (match?.[1]) {
            postId = match[1];
            ronsLogger.log(`Extracted ID: ${postId}`);
        } else {
            ronsLogger.log("No ID found.");
        }
        ...
        if (!isDKGMemoryContent(memoryKnowledgeGraph.object)) {
            ronsLogger.error("Invalid DKG memory content generated.");
        ...
        try {
            ronsLogger.log("Publishing message to DKG");
            ...
            ronsLogger.log("======================== ASSET CREATED");
            ronsLogger.log(JSON.stringify(createAssetResult));
        } catch (error) {
            ronsLogger.error(
