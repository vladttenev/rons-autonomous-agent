...
export class SampleService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ...
        ronsLogger.info("SampleService initialized and started periodic task");
    }
    ...
    private async fetchSample(): Promise<void> {
        if (!this.runtime) {
            ronsLogger.error("SampleService: Runtime not initialized");
            return;
        }
        ...
        try {
            ...
            ronsLogger.info("SampleService: Hello world");

            ronsLogger.info(
                "SampleService: Successfully fetched and processed sample"
            );
        } catch (error) {
            ronsLogger.error("SampleService: Error fetching sample:", error);
        }
    }
