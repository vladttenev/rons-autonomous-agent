# @rons-org/plugin-conflux

A plugin for interacting with the Conflux blockchain network within the RonsOS ecosystem.
...
```bash
pnpm install @rons-org/plugin-conflux
```
...
```typescript
import { confluxPlugin } from "@rons-org/plugin-conflux";
```
