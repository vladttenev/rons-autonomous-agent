...
import { generateObject, composeContext, ModelClass } from "@rons-org/core";
import {
    createPublicClient,
    createWalletClient,
    http,
...
async function ensureAllowance(
    ...
    ronsLogger.log("allowance:", allowance);
    ...
    if (allowance < amount) {
        ...
        ronsLogger.log(`Approving hash: ${hash}`);
        await publicClient.waitForTransactionReceipt({ hash });
        ronsLogger.log(`Approving success: ${hash}`);
    } else {
        ronsLogger.log("No need to approve");
    }
}
...
export const confiPump: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            ronsLogger.log("simulate: ", simulate);
            ...
            ronsLogger.error(`Error performing the action: ${error}`);
            if (callback) {
                callback({
