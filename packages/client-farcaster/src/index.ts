...
class FarcasterManager {
    ...
    constructor(runtime: IAgentRuntime, farcasterConfig: FarcasterConfig) {
        ...
        ronsLogger.success("Farcaster Neynar client initialized.");
...
export const FarcasterClientInterface: Client = {
    async start(runtime: IAgentRuntime) {
        ...
        ronsLogger.log("Farcaster client started");
    ...
    async stop(runtime: IAgentRuntime) {
        try {
            // stop it
            ronsLogger.log("Stopping farcaster client", runtime.agentId);
            if (runtime.clients.farcaster) {
                await runtime.clients.farcaster.stop();
            }
        } catch (e) {
            ronsLogger.error("client-farcaster interface stop error", e);
        }
    },
};
