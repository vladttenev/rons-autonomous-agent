...
export class LensClient {
    ...
    async authenticate(): Promise<void> {
        try {
            ...
            ronsLogger.error("client-lens::client error: ", error);
            throw error;
        }
    }
    ...
    async createPublication(
        ...
        try {
            if (!this.authenticated) {
                ...
                ronsLogger.log("done authenticating");
            }
            let broadcastResult;
            ...
            ronsLogger.log("broadcastResult", broadcastResult);
    ...
    async getProfile(profileId: string): Promise<Profile> {
        ...
        if (!result?.id) {
            ronsLogger.error("Error fetching user by profileId");
    ...
    private async createPostMomoka(
        ...
        ronsLogger.log("createPostMomoka");
        // gasless + signless if they enabled the lens profile manager
        if (this.authenticatedProfile?.signless) {
        ...
        ronsLogger.log("typedDataResult", typedDataResult);
        const { id, typedData } = typedDataResult.unwrap();
