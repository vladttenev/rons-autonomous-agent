...
export class LensPostManager {
    ...
    private async generateNewPublication() {
        ronsLogger.info("Generating new publication");
        try {
            ...
            if (this.runtime.getSetting("LENS_DRY_RUN") === "true") {
                ronsLogger.info(`Dry run: would have posted: ${content}`);
                return;
            }
            ...
            try {
                ...
                ronsLogger.info(`[Lens Client] Published ${publication.id}`);
                ...
                ronsLogger.error("Error sending publication:", error);
            }
        } catch (error) {
            ronsLogger.error("Error generating new publication:", error);
        }
    }
}
