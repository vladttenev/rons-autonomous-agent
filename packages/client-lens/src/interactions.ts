...
export class LensInteractionManager {
    ...
    private async handleInteractions() {
        ronsLogger.info("Handle Lens interactions");
    ...
    private async handlePublication({
        ...
        if (publication.by.id === agent.id) {
            ronsLogger.info("skipping cast from bot itself", publication.id);
            return;
        }

        if (!memory.content.text) {
            ronsLogger.info("skipping cast with no text", publication.id);
