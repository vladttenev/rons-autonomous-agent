# @rons-org/plugin-open-weather

A plugin for Rons that enables weather checking using the OpenWeather API.
...
```bash
npm install @rons-org/plugin-open-weather
```
...
```typescript
import { openWeatherPlugin } from "@rons-org/plugin-open-weather";
