...
export class DeployAction {
    ...
    async compileSolidity(contractName: string, source: string) {
        ...
        ronsLogger.debug("Compiling contract...");
        ...
        if (!contract) {
            ronsLogger.error("Compilation result is empty");
        }

        ronsLogger.debug("Contract compiled successfully");
        return {
    ...
    async deployERC20(deployTokenParams: IDeployERC20Params) {
        ronsLogger.debug("deployTokenParams", deployTokenParams);
        ...
        try {
            ...
            ronsLogger.error("Depoly ERC20 failed:", error.message);
            throw error;
        }
    }

    async deployERC721(deployNftParams: IDeployERC721Params) {
        ronsLogger.debug("deployNftParams", deployNftParams);
        ...
        try {
            ...
            ronsLogger.error("Depoly ERC721 failed:", error.message);
            throw error;
        }
    }

    async deployERC1155(deploy1155Params: IDeployERC1155Params) {
        ronsLogger.debug("deploy1155Params", deploy1155Params);
        ...
        try {
            ...
            ronsLogger.error("Depoly ERC1155 failed:", error.message);
            throw error;
        }
    }
    ...
    async deployContract(
        ...
        ronsLogger.debug("Waiting for deployment transaction...", hash);
        const publicClient = this.walletProvider.getPublicClient(chain);
        const receipt = await publicClient.waitForTransactionReceipt({
            hash,
        });
        ronsLogger.debug("Contract deployed successfully!");
...
export const deployAction = {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting deploy action...");
        ...
        try {
            ...
            ronsLogger.error("Error during deploy:", error.message);
            callback?.({
