...
export class FaucetAction {
    ...
    async faucet(params: FaucetParams): Promise<FaucetResponse> {
        ronsLogger.debug("Faucet params:", params);
        await this.validateAndNormalizeParams(params);
        ronsLogger.debug("Normalized faucet params:", params);
...
export const faucetAction = {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting faucet action...");
        ...
        try {
            ...
            ronsLogger.error("Error during faucet:", error.message);
            callback?.({
