...
The SGX Plugin for Rons enhances the platform by providing Intel SGX attestation capabilities.
...
After starting Rons, the provider `sgxAttestationProvider` will be registered into Rons through plugin-sgx. The environment variable `SGX` is required to be set to `1` to enable plugin-sgx. And the `SGX` is always set to `1` in the SGX environment through the `rons.manifest.template` file.

When Rons starts, the `sgxAttestationProvider` will generate SGX attestation in each request. And you can use the `SgxAttestationProvider` to generate SGX remote attestations for your own plugins / clients.
...
```bash
sudo docker run -it --name rons_sgx \
    --mount type=bind,source={your_rons_path},target=/root/rons \
    --device /dev/sgx/enclave \
    --device /dev/sgx/provision \
    gramineproject/gramine:stable-jammy
```

After entering the docker, you can use the following command to prepare the Rons environment:
...
# Gramine will utilize this path as the default Node.js location to run Rons.
