...
export default {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting ESCROW_OPERATION handler...");
        ...
        if (!isEscrowContent(content)) {
            ronsLogger.error("Invalid content for ESCROW_OPERATION action.");
            callback?.({
        ...
        try {
            ...
            ronsLogger.log(`Current ${content.token} balance:`, balance);
            ...
            if (content.operation === "check") {
                ...
                try {
                    ...
                    ronsLogger.error("Deposit operation failed:", error);
                    callback?.({
                ...
                try {
                    ...
                    ronsLogger.error("Withdraw operation failed:", error);
                    callback?.({
            ...
            ronsLogger.error("Escrow operation failed:", error);
            callback?.({
