# Spheron Protocol Plugin for Rons

This plugin integrates the Spheron Protocol SDK into the Rons ecosystem, providing functionality for managing deployments, escrow operations, and lease management.
...
```bash
npm install @rons-org/plugin-spheron
```
...
```typescript
import { spheronPlugin } from "@rons-org/plugin-spheron";

// Register with Rons
rons.registerPlugin(spheronPlugin);
```
...
1. You can ask the rons to deploy a template like `jupyter-notebook`, `vscode`, `ollama`, or `heurist-miner` for you with just natural language.
2. You can also customize the deployment with natural language.
3. You can also ask the rons to close the deployment.
4. You can also ask the rons to check the balance of your account.
5. You can also ask the rons to deposit or withdraw tokens from your account.
