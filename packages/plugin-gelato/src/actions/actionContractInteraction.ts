...
export class ContractInteractionAction {
    async interactWithContract(
        ...
        ronsLogger.info(`Interacting with contract at ${target} on ${chain}`);
...
export const contractInteractionAction: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            ronsLogger.info(`Function: ${validatedContent.functionName}`);
            ronsLogger.info(`Target: ${validatedContent.target}`);
            ronsLogger.info(`Chain: ${validatedContent.chain}`);
            ronsLogger.info(`Parsed ABI: ${JSON.stringify(parsedAbi)}`);
            if (parsedContent.user) {
                ronsLogger.info(`User: ${parsedContent.user}`);
            }
