...
export const JeeterClientInterface: Client = {
    async start(runtime: IAgentRuntime) {
        if (activeManager) {
            ronsLogger.warn("SimsAI client already started");
            return activeManager;
        }

        await validateJeeterConfig(runtime);

        ronsLogger.log("SimsAI client started");
    ...
    async stop(_runtime: IAgentRuntime) {
        ronsLogger.log("Stopping SimsAI client");
        if (activeManager) {
            try {
                ...
                activeManager = null;
                ronsLogger.log("SimsAI client stopped successfully");
            } catch (error) {
                ronsLogger.error("Error stopping SimsAI client:", error);
                throw error;
            }
        }
        ronsLogger.log("SimsAI client stopped");
    },
};
