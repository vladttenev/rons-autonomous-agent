...
export class JeeterPostClient {
    ...
    async start(postImmediately: boolean = false) {
        if (this.isRunning) {
            ronsLogger.warn("JeeterPostClient is already running");
            return;
        }
        ...
        const generateNewJeetLoop = async () => {
            if (!this.isRunning) {
                ronsLogger.log("JeeterPostClient has been stopped");
                return;
            }
            ...
            try {
                ...
                ronsLogger.error("Error in generateNewJeetLoop:", error);
                if (this.isRunning) {
                    this.timeoutHandle = setTimeout(
                        () => {
                            generateNewJeetLoop();
    ...
    public async stop() {
        ronsLogger.log("Stopping JeeterPostClient...");
        ...
        ronsLogger.log("JeeterPostClient stopped successfully");
    }
...
${timestamp}\n\n${jeet.text}\n---\n`;
    ...
    private async postJeet(content: string): Promise<Jeet> {
        ...
        ronsLogger.log(`Jeet posted with ID: ${response.data.id}`);
    ...
    private async generateNewJeet() {
        if (!this.isRunning) {
            ronsLogger.log("Skipping jeet generation - client is stopped");
            return;
        }

        ronsLogger.log("Generating new jeet");
        try {
            ...
            if (dryRun === "true" || dryRun === "1") {
                ronsLogger.info(`Dry run: would have posted jeet: ${content}`);
                return;
            }
            ...
            try {
                ...
                ronsLogger.log(`Posting new jeet:\n ${content}`);
                ...
                await this.client.cacheTimeline(homeTimeline);
                ronsLogger.log(`Jeet posted at: ${jeet.permanentUrl}`);
                await this.createMemoryForJeet(jeet, content);
            } catch (error) {
                ronsLogger.error("Error sending jeet:", error);
            ...
            ronsLogger.error("Error generating new jeet:", error);
            if (error instanceof Error) {
                ronsLogger.error("Error details:", {
                    message: error.message,
                    stack: error.stack,
