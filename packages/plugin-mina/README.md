# @rons-org/plugin-mina

Core Mina blockchain plugin for Rons OS that provides essential services and actions for token operations and wallet management.
...
```bash
npm install @rons-org/plugin-mina
```
...
```typescript
import { minaPlugin } from "@rons-org/plugin-mina";
