...
`,
        ...
        ronsLogger.error("[generateFiller] Error generating filler:", err);
        return "";
    }
}
...
async function speakFiller(
    ...
    ronsLogger.log(`[Space] Filler (${fillerType}) => ${text}`);
...
`,
        ...
        ronsLogger.error("[generateTopicsIfEmpty] GPT error =>", err);
...
export class TwitterSpaceClient {
    ...
    public async startPeriodicSpaceCheck() {
        ronsLogger.log("[Space] Starting periodic check routine...");
        ...
        const routine = async () => {
            try {
                ...
                ronsLogger.error("[Space] Error in routine =>", error);
    ...
    private async shouldLaunchSpace(): Promise<boolean> {
        ...
        if (r > (this.decisionOptions.randomChance ?? 0.3)) {
            ronsLogger.log("[Space] Random check => skip launching");
            return false;
        }
        // Business hours
        if (this.decisionOptions.businessHoursOnly) {
            ...
            if (hour < 9 || hour >= 17) {
                ronsLogger.log("[Space] Out of business hours => skip");
                return false;
            }
        }
        // Interval
        ...
        if (this.lastSpaceEndedAt) {
            ...
            if (now - this.lastSpaceEndedAt < minIntervalMs) {
                ronsLogger.log("[Space] Too soon since last space => skip");
                return false;
            }
        }

        ronsLogger.log("[Space] Deciding to launch a new Space...");
        return true;
    }
    ...
    public async startSpace(config: SpaceConfig) {
        ronsLogger.log("[Space] Starting a new Twitter Space...");
        ...
        try {
            ...
            if (this.decisionOptions.enableRecording) {
                ronsLogger.log("[Space] Using RecordToDiskPlugin");
                this.currentSpace.use(new RecordToDiskPlugin());
            }

            if (this.decisionOptions.enableSttTts) {
                ronsLogger.log("[Space] Using SttTtsPlugin");
            ...
            if (this.decisionOptions.enableIdleMonitor) {
                ronsLogger.log("[Space] Using IdleMonitorPlugin");
            ...
            ronsLogger.log(`[Space] Space started => ${spaceUrl}`);
            ...
            process.on("SIGINT", async () => {
                ronsLogger.log("[Space] SIGINT => stopping space");
                await speakFiller(
                    this.client.runtime,
                    this.sttTtsPlugin,
                    "CLOSING"
            ...
            ronsLogger.error("[Space] Error launching Space =>", error);
            this.isSpaceRunning = false;
            throw error;
        }
    }
    ...
    private async manageCurrentSpace() {
        ...
        try {
            ...
            ronsLogger.error("[Space] Error in manageCurrentSpace =>", error);
        }
    }
    ...
    private async handleSpeakerRequest(req: SpeakerRequest) {
        ...
        if (janusSpeakers.length < (this.decisionOptions.maxSpeakers ?? 1)) {
            ronsLogger.log(`[Space] Accepting speaker @${req.username} now`);
            await speakFiller(
                this.client.runtime,
                this.sttTtsPlugin,
                "PRE_ACCEPT"
    ...
    private async acceptSpeaker(req: SpeakerRequest) {
        ...
        try {
            ...
            ronsLogger.log(`[Space] Speaker @${req.username} is now live`);
        } catch (err) {
            ronsLogger.error(
    ...
    private async removeSpeaker(userId: string) {
        ...
        try {
            ...
            ronsLogger.log(`[Space] Removed speaker userId=${userId}`);
        } catch (error) {
            ronsLogger.error(
    ...
    public async stopSpace() {
        ...
        try {
            ronsLogger.log("[Space] Stopping the current Space...");
            await this.currentSpace.stop();
        } catch (err) {
            ronsLogger.error("[Space] Error stopping Space =>", err);
        } finally {
            this.isSpaceRunning = false;
            this.spaceId = undefined;
