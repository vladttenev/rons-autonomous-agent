...
export class SttTtsPlugin implements Plugin {
    ...
    onAttach(_space: Space) {
        ronsLogger.log("[SttTtsPlugin] onAttach => space was attached");
    }
        ...
        if (!this.isSpeaking) {
            ...
            if (avgVolume > SPEAKING_THRESHOLD) {
                ...
                if (this.ttsAbortController) {
                    ...
                    ronsLogger.log("[SttTtsPlugin] TTS playback interrupted");
                }
            }
        }
    }
    ...
    private async processAudio(userId: string): Promise<void> {
        ...
        try {
            ...
            ronsLogger.error("[SttTtsPlugin] processAudio error =>", error);
        } finally {
            this.isProcessingAudio = false;
        }
    }
    ...
    private async processTtsQueue(): Promise<void> {
        while (this.ttsQueue.length > 0) {
            ...
            try {
                ...
                ronsLogger.error("[SttTtsPlugin] TTS streaming error =>", err);
            } finally {
    ...
    private async _shouldIgnore(message: Memory): Promise<boolean> {
        ronsLogger.debug("message.content: ", message.content);
    ...
    private async streamToJanus(
        ...
        for (
            ...
            if (this.ttsAbortController?.signal.aborted) {
                ronsLogger.log("[SttTtsPlugin] streamToJanus interrupted");
                return;
            }
    ...
    public clearChatContext() {
        this.chatContext = [];
        ronsLogger.log("[SttTtsPlugin] clearChatContext => done");
    }

    cleanup(): void {
        ronsLogger.log("[SttTtsPlugin] cleanup => releasing resources");
        this.pcmBuffers.clear();
        this.userSpeakingTimer = null;
        this.ttsQueue = [];
        this.isSpeaking = false;
