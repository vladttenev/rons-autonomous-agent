...
Core Sui blockchain plugin for Rons OS that provides essential services and actions for token operations and wallet management.
...
```bash
npm install @rons-org/plugin-sui
```
...
```typescript
import { suiPlugin } from "@rons-org/plugin-sui";
