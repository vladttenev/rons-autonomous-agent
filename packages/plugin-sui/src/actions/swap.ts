...
export default {
    ...
    handler: async (
        ...
        ronsLogger.info("Swap content:", swapContent);
        ...
        if (service.getNetwork() == "mainnet") {
            ...
            ronsLogger.log("Destination token:", destinationToken);

            const fromToken = await service.getTokenMetadata(
                swapContent.from_token
            );

            ronsLogger.log("From token:", fromToken);
            ...
            if (destinationToken && fromToken) {
                try {
                    ...
                    ronsLogger.info("Swap amount:", swapAmount);
                    ...
                    ronsLogger.error("Error swapping token:", error);
                    callback({
