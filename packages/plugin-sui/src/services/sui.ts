...
export class SuiService extends Service {
    ...
    async swapToken(
        ...
        ronsLogger.info("From token metadata:", fromMeta);
        ronsLogger.info("To token metadata:", toMeta);
        ...
        if (fromToken.toUpperCase() === "SUI") {
            ...
            if (allCoins.data.length === 0) {
                ronsLogger.error("No coins found");
                return {
                    success: false,
                    tx: "",
            ...
            for (let i = 1; i < allCoins.data.length; i++) {
                ronsLogger.info("Coin:", allCoins.data[i]);
                mergeCoins.push(allCoins.data[i].coinObjectId);
            }
            ronsLogger.info("Merge coins:", mergeCoins);
