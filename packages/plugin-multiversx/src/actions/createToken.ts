...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ronsLogger.log("Validating config for user:", message.userId);
    ...
    handler: async (
        ...
        ronsLogger.log("Starting CREATE_TOKEN handler...");

        ronsLogger.log("Handler initialized. Checking user authorization...");
        ...
        if (!isCreateTokenContent) {
            ronsLogger.error("Invalid content for CREATE_TOKEN action.");
            if (callback) {
                callback({
        ...
        try {
            ...
            ronsLogger.error("Error during creating token:", error);
            if (callback) {
                callback({
