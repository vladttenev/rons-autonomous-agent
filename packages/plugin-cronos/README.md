# @rons-org/plugin-cronos

Cronos plugin for Rons, extending the EVM plugin functionality.
...
```bash
pnpm add @rons-org/plugin-cronos
```
...
### Basic Setup
```typescript
import { cronosPlugin } from "@rons-org/plugin-cronos";

// Use the plugin in your Rons configuration
const config = {
...
```json
{
    ...
    "plugins": ["@rons-org/plugin-cronos"],
    "clients": ["telegram"],
...
1. **Basic Setup**
   ...
   - `plugins`: Include `@rons-org/plugin-cronos`
...
- All standard EVM functionality inherited from @rons-org/plugin-evm
