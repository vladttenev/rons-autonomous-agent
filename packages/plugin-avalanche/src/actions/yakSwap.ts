...
function isSwapContent(
    ...
    ronsLogger.debug("Content for swap", content);
    return (
...
export default {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting SWAP_TOKEN handler...");
        ...
        if (!isSwapContent(runtime, content)) {
            ronsLogger.error("Invalid content for SWAP_TOKEN action.");
            callback?.({
        ...
        ronsLogger.debug("Swap content:", content);
        ...
        if (
            ...
            ronsLogger.log("Swapping from native AVAX");
        } else if (
            content.toTokenAddress ===
            "0x0000000000000000000000000000000000000000"
        ) {
            // todo: swap to native
            ronsLogger.log("Swapping to native AVAX");
        } else {
            ...
            if (tx) {
                ...
                if (receipt.status === "success") {
                    ...
                    if (swapTx) {
                        ...
                        if (receipt.status === "success") {
                            ronsLogger.log("Swap successful");
                            callback?.({
                                text: "swap successful",
                                content: { success: true, txHash: swapTx },
                            });
                        } else {
                            ronsLogger.error("Swap failed");
                            callback?.({
                                text: "swap failed",
                    ...
                    ronsLogger.error("Approve failed");
                    callback?.({
                        text: "approve failed",
