...
import { ronsLogger, type Memory, type Content } from '@rons-org/core';

// Increase test timeout
vi.setConfig({ testTimeout: 15000 });

// Mock the @rons-org/core module
vi.mock('@rons-org/core', () => ({
    ronsLogger: {
...
describe('PostgresDatabaseAdapter - Vector Extension Validation', () => {
    ...
    const initializeDatabase = async (client: pg.PoolClient) => {
        ronsLogger.info('Initializing database with schema...');
        try {
            // Set app settings for vector dimension
            await client.query(`
                ALTER DATABASE rons_test SET app.use_openai_embedding = 'true';
                ALTER DATABASE rons_test SET app.use_ollama_embedding = 'false';
            `);
            ...
            `);
            ronsLogger.info('Vector extension status:', { isInstalled: vectorExt.length > 0 });

            const { rows: dimension } = await client.query('SELECT get_embedding_dimension()');
            ronsLogger.info('Vector dimension:', { dimension: dimension[0].get_embedding_dimension });

            // Verify search path
            const { rows: searchPath } = await client.query('SHOW search_path');
            ronsLogger.info('Search path:', { searchPath: searchPath[0].search_path });

        } catch (error) {
            ronsLogger.error(`Database initialization failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw error;
        }
    };

    const cleanDatabase = async (client: pg.PoolClient) => {
        ronsLogger.info('Starting database cleanup...');
        try {
            ...
            ronsLogger.success('Database cleanup completed successfully');
        } catch (error) {
            ronsLogger.error(`Database cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw error;
        }
    };

    beforeAll(async () => {
        ronsLogger.info('Setting up test database...');
    ...
    beforeEach(async () => {
        ronsLogger.info('Setting up test environment...');
        try {
            ...
            ronsLogger.debug('Database connection established');

            await cleanDatabase(testClient);
            ronsLogger.debug('Database cleaned');
            ...
            ronsLogger.success('Test environment setup completed');
        } catch (error) {
            ronsLogger.error(`Test environment setup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw error;
        }
    });

    afterEach(async () => {
        ronsLogger.info('Cleaning up test environment...');
        try {
            await cleanDatabase(testClient);
            await testClient?.release();
            await testPool?.end();
            await adapter?.close();
            ronsLogger.success('Test environment cleanup completed');
        } catch (error) {
            ronsLogger.error(`Test environment cleanup failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
            throw error;
        }
    });
    ...
    describe('Schema and Extension Management', () => {
        test('should initialize with vector extension', async () => {
            ronsLogger.info('Testing vector extension initialization...');
            try {
                // Act
                ronsLogger.debug('Initializing adapter...');
                await adapter.init();
                ronsLogger.success('Adapter initialized successfully');

                // Assert
                ronsLogger.debug('Verifying vector extension existence...');
                const { rows } = await testClient.query(`
                    SELECT 1 FROM pg_extension WHERE extname = 'vector'
                `);
                expect(rows.length).toBe(1);
                ronsLogger.success('Vector extension verified successfully');
            } catch (error) {
                ronsLogger.error(`Vector extension test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        });

        test('should handle missing rooms table', async () => {
            ronsLogger.info('Testing rooms table creation...');
            try {
                // Act
                ronsLogger.debug('Initializing adapter...');
                await adapter.init();
                ...
                // Assert
                ronsLogger.debug('Verifying rooms table existence...');
                const { rows } = await testClient.query(`
                    SELECT EXISTS (
                ...
                ronsLogger.success('Rooms table verified successfully');
            } catch (error) {
                ronsLogger.error(`Rooms table test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        });

        test('should not reapply schema when everything exists', async () => {
            ronsLogger.info('Testing schema reapplication prevention...');
            try {
                // Arrange
                ronsLogger.debug('Setting up initial schema...');
                await adapter.init();
                ronsLogger.success('Initial schema setup completed');

                const spy = vi.spyOn(fs, 'readFileSync');
                ronsLogger.debug('File read spy installed');

                // Act
                ronsLogger.debug('Attempting schema reapplication...');
                await adapter.init();
                ronsLogger.success('Second initialization completed');

                // Assert
                expect(spy).not.toHaveBeenCalled();
                ronsLogger.success('Verified schema was not reapplied');
                spy.mockRestore();
            } catch (error) {
                ronsLogger.error(`Schema reapplication test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        });

        test('should handle transaction rollback on error', async () => {
            ronsLogger.info('Testing transaction rollback...');
            try {
                // Arrange
                ronsLogger.debug('Setting up file read error simulation...');
                const spy = vi.spyOn(fs, 'readFileSync').mockImplementation(() => {
                    ronsLogger.warn('Simulating schema read error');
                    throw new Error('Schema read error');
                });

                // Act & Assert
                ronsLogger.debug('Attempting initialization with error...');
                await expect(adapter.init()).rejects.toThrow('Schema read error');
                ronsLogger.success('Error thrown as expected');

                // Verify no tables were created
                ronsLogger.debug('Verifying rollback...');
                const { rows } = await testClient.query(`
                    SELECT EXISTS (
                        SELECT FROM information_schema.tables
                        WHERE table_name = 'rooms'
                ...
                ronsLogger.success('Rollback verified successfully');
                spy.mockRestore();
            } catch (error) {
                ronsLogger.error(`Transaction rollback test failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
                throw error;
            }
        });
    });
    ...
    describe('Memory Operations with Vector', () => {
        ...
        beforeEach(async () => {
            ronsLogger.info('Setting up memory operations test...');
            try {
                ...
                ronsLogger.error('Memory operations setup failed:', {
