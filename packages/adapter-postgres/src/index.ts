    ...
    implements IDatabaseCacheAdapter
{
    ...
    constructor(connectionConfig: any) {
        ...
        this.pool.on("error", (err) => {
            ronsLogger.error("Unexpected pool error", err);
        ...
        for (let attempt = 1; attempt <= this.maxRetries; attempt++) {
            try {
                ...
                if (attempt < this.maxRetries) {
                    ...
                    ronsLogger.error("Max retry attempts reached:", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
    ...
    private async handlePoolError(error: Error) {
        ronsLogger.error("Pool error occurred, attempting to reconnect", {
            error: error.message,
        });
        ...
        try {
            ...
            ronsLogger.success("Pool reconnection successful");
        } catch (reconnectError) {
            ronsLogger.error("Failed to reconnect pool", {
                error:
    ...
    private async validateVectorSetup(): Promise<boolean> {
        try {
            ...
            if (!hasVector) {
                ronsLogger.error("Vector extension not found in database");
                return false;
            }

            return true;
        } catch (error) {
            ronsLogger.error("Failed to validate vector extension:", {
    ...
    async testConnection(): Promise<boolean> {
        ...
        try {
            ...
            ronsLogger.error("Database connection test failed:", error);
            throw new Error(
    ...
    async cleanup(): Promise<void> {
        try {
            await this.pool.end();
            ronsLogger.info("Database pool closed");
        } catch (error) {
            ronsLogger.error("Error closing database pool:", error);
        }
    }
    ...
    async getAccountById(userId: UUID): Promise<Account | null> {
        return this.withDatabase(async () => {
            ...
            if (rows.length === 0) {
                ronsLogger.debug("Account not found:", { userId });
                return null;
            }

            const account = rows[0];
            // ronsLogger.debug("Account retrieved:", {
            //     userId,
    ...
    async createAccount(account: Account): Promise<boolean> {
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.debug("Account created successfully:", {
                    accountId,
                });
                return true;
            } catch (error) {
                ronsLogger.error("Error creating account:", {
                    error:
    ...
    async getActorById(params: { roomId: UUID }): Promise<Actor[]> {
        return this.withDatabase(async () => {
            ...
            ronsLogger.debug("Retrieved actors:", {
            ...
            return rows.map((row) => {
                try {
                    ...
                    ronsLogger.warn("Failed to parse actor details:", {
                        actorId: row.id,
                        error:
                            error instanceof Error
                                ? error.message
            ...
            ronsLogger.error("Failed to get actors:", {
                roomId: params.roomId,
                error: error.message,
            });
    ...
    async createMemory(memory: Memory, tableName: string): Promise<void> {
        return this.withDatabase(async () => {
            ronsLogger.debug("PostgresAdapter createMemory:", {
                memoryId: memory.id,
    ...
    async getMemories(params: {
        ...
        return this.withDatabase(async () => {
            ...
            ronsLogger.debug("Fetching memories:", {
                roomId: params.roomId,
                tableName: params.tableName,
                unique: params.unique,
                agentId: params.agentId,
    ...
    async updateGoal(goal: Goal): Promise<void> {
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.error("Failed to update goal:", {
                    goalId: goal.id,
                    error:
                        error instanceof Error ? error.message : String(error),
                    status: goal.status,
    ...
    async removeGoal(goalId: UUID): Promise<void> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.debug("Goal removal attempt:", {
                    goalId,
                    removed: result?.rowCount ?? 0 > 0,
                });
            } catch (error) {
                ronsLogger.error("Failed to remove goal:", {
                    goalId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
        ...
        return this.withDatabase(async () => {
            ...
            try {
                ...
                if (checkResult.rowCount === 0) {
                    ronsLogger.warn("No room found to remove:", { roomId });
                ...
                await client.query("ROLLBACK");
                ronsLogger.error("Failed to remove room:", {
                    roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async createRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.debug("Relationship created successfully:", {
                    relationshipId,
                    userA: params.userA,
                    userB: params.userB,
                });
                ...
                if ((error as { code?: string }).code === "23505") {
                    // Unique violation
                    ronsLogger.warn("Relationship already exists:", {
                        userA: params.userA,
                        userB: params.userB,
                        error:
                            error instanceof Error
                    ...
                    ronsLogger.error("Failed to create relationship:", {
                        userA: params.userA,
                        userB: params.userB,
                        error:
                            error instanceof Error
    ...
    async getRelationship(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                if (rows.length > 0) {
                    ronsLogger.debug("Relationship found:", {
                ...
                ronsLogger.debug("No relationship found between users:", {
                    userA: params.userA,
                    userB: params.userB,
                });
                return null;
            } catch (error) {
                ronsLogger.error("Error fetching relationship:", {
                    userA: params.userA,
                    userB: params.userB,
                    error:
                        error instanceof Error ? error.message : String(error),
    ...
    async getRelationships(params: { userId: UUID }): Promise<Relationship[]> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.debug("Retrieved relationships:", {
                ...
                ronsLogger.error("Failed to fetch relationships:", {
                    userId: params.userId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getCachedEmbeddings(opts: {
        ...
        return this.withDatabase(async () => {
            try {
                ronsLogger.debug("Fetching cached embeddings:", {
                ...
                ronsLogger.debug("Retrieved cached embeddings:", {
                    count: rows.length,
                    tableName: opts.query_table_name,
                    matchCount: opts.query_match_count,
                });
                ...
                ronsLogger.error("Error in getCachedEmbeddings:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    tableName: opts.query_table_name,
                    fieldName: opts.query_field_name,
    ...
    async log(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.debug("Log entry created:", {
                    logId,
                    type: params.type,
                    roomId: params.roomId,
                    userId: params.userId,
                ...
                ronsLogger.error("Failed to create log entry:", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    type: params.type,
                    roomId: params.roomId,
    ...
    async searchMemoriesByEmbedding(
        ...
        return this.withDatabase(async () => {
            ronsLogger.debug("Incoming vector:", {
    ...
    async getActorDetails(params: { roomId: string }): Promise<Actor[]> {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.debug("Retrieved actor details:", {
                    roomId: params.roomId,
                ...
                ronsLogger.error("Failed to fetch actor details:", {
                    roomId: params.roomId,
                    error:
                        error instanceof Error ? error.message : String(error),
                });
    ...
    async getCache(params: {
        ...
        return this.withDatabase(async () => {
            try {
                ...
                ronsLogger.error("Error fetching cache", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    key: params.key,
                    agentId: params.agentId,
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    ...
                    await client.query("ROLLBACK");
                    ronsLogger.error("Error setting cache", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
        ...
        return this.withDatabase(async () => {
            try {
                ...
                try {
                    ...
                    await client.query("ROLLBACK");
                    ronsLogger.error("Error deleting cache", {
                        error:
                            error instanceof Error
                                ? error.message
                                : String(error),
    ...
    async removeKnowledge(id: UUID): Promise<void> {
        return this.withDatabase(async () => {
            ...
            try {
                ...
                await client.query("ROLLBACK");
                ronsLogger.error("Error removing knowledge", {
                    error:
                        error instanceof Error ? error.message : String(error),
                    id,
                });
