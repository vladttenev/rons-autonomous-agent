# @rons-org/plugin-near

NEAR Protocol integration plugin for Rons OS that enables token management, transfers, and swaps using Ref Finance.
...
```bash
pnpm install @rons-org/plugin-near
```
...
```typescript
import { nearPlugin } from "@rons-org/plugin-near";
...
```typescript
const walletInfo = await rons.getProvider("wallet");
