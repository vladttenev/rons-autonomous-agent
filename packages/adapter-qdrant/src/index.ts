...
export class QdrantDatabaseAdapter  extends DatabaseAdapter<QdrantClient>  implements IDatabaseCacheAdapter {
    ...
    constructor(url: string, apiKey: string, port: number, vectorSize: number) {
        super();
        ronsLogger.info("new Qdrant client...");
    ...
    private preprocess(content: string): string {
        if (!content || typeof content !== "string") {
            ronsLogger.warn("Invalid input for preprocessing");
            return "";
        }
    ...
    async init () {
        ...
        if (collectionNames.includes(this.collectionName)) {
            ronsLogger.info("Collection already exists.");
        } else {
            ronsLogger.info("create collection...");
    ...
    async createKnowledge(knowledge: RAGKnowledgeItem): Promise<void> {
        ...
        ronsLogger.info("Qdrant adapter createKnowledge id:", knowledge.id);
    ...
    async getKnowledge(params: {
        ...
        ronsLogger.info("Qdrant adapter getKnowledge...", params.id);
    ...
    async searchKnowledge(params: {
        ...
        const results: RAGKnowledgeItem[] = rows.map((row) => {
            ...
            ? JSON.parse(row.payload.content)
            : row.payload?.content;
            ronsLogger.info("Qdrant adapter searchKnowledge  id:", row.id.toString() as UUID);
            return {
                id: row.id.toString() as UUID,
                agentId: (row.payload?.agentId || "") as UUID,
                content: {
        ...
        ronsLogger.debug("Qdrant adapter searchKnowledge results:", results);
        await this.setCache({
            key: cacheKey,
            agentId: params.agentId,
