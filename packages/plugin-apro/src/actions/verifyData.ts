...
export const verifyData: Action = {
  ...
  handler: async (
    ...
    try {
        ...
        ronsLogger.info('The verify params received:', verifyParams);
    }  catch (error: any) {
        ronsLogger.error('Failed to generate verify params:', error);
        callback({
    ...
    if (!isVerifyParams(verifyParams)) {
        ronsLogger.error('Invalid verify params:', verifyParams);
        callback({
    ...
    try {
        ...
        ronsLogger.info(`Data verification transaction sent. Transaction ID: ${tx.hash}`);

        const receipt = await tx.wait();
        ronsLogger.info(`Data verified successfully.`);

        callback({
            text: 'Success: Data verified successfully. Transaction ID: ' + receipt.hash,
        })
    } catch (error: any) {
        ronsLogger.error(`Error verify data: ${error.message}`);
