import { Action, composeContext, ronsLogger, generateObject, HandlerCallback, IAgentRuntime, Memory, ModelClass, State } from "@rons-org/core";
...
export const createAndRegisterAgent: Action = {
  ...
  handler: async (
    ...
    try {
        ...
        ronsLogger.info('The Agent settings received:', agentSettings);
    } catch (error: any) {
        ronsLogger.error('Failed to generate Agent settings:', error);
        callback({
    ...
    if (!isAgentSettings(agentSettings)) {
        ronsLogger.error('Invalid Agent settings:', agentSettings);
        callback({
    ...
    try {
        ...
        ronsLogger.error('Failed to create Agent SDK:', error);
        callback({
    ...
    try {
        ...
        ronsLogger.info('Successfully send create and register agent transaction:', tx.hash);

        const receipt = await tx.wait()
        const agentAddress = parseNewAgentAddress(receipt)

        ronsLogger.info(`Created agent at address: ${agentAddress}`)
        callback({ text: 'Agent created and registered successfully: ' + agentAddress })
    } catch (error: any) {
        ronsLogger.error(`Error creating agent: ${error.message}`);
          ...
          text: `I want to Create and register apro ai-agent with the following settings:
            {
                ...
                agentHeader: {
                    sourceAgentName: 'RonsOS Test Agent',
