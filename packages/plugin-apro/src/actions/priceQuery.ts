...
export const priceQuery: Action = {
    ...
    handler: async (
        ...
        try {
            ...
            ronsLogger.info('The price query params received:', priceQueryParams);
        } catch (error: any) {
            ronsLogger.error('Failed to generate price query params:', error);
            callback({
        ...
        if (!isPriceQueryParams(priceQueryParams)) {
            ronsLogger.error('Invalid price query params:', priceQueryParams);
            callback({
        ...
        try {
            ...
            ronsLogger.info('The Price data received:', priceData);

            if (!priceData || priceData.length === 0) {
                ronsLogger.error('No price data found for pair:', pair);
                callback({
            ...
            ronsLogger.error(`Error fetching price data, error: `, error);
            callback(
                {
