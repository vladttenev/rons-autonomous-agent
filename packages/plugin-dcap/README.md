...
- Generate DCAP attestation on TDX using the `remoteAttestationProvider` provided by the [plugin-tee](https://github.com/ronsOS/rons/tree/develop/packages/plugin-tee).
- Generate DCAP attestation on SGX using the `sgxAttestationProvider` provided by the [plugin-sgx](https://github.com/ronsOS/rons/tree/develop/packages/plugin-sgx).
...
```bash
pnpm install @rons-org/plugin-dcap
```
...
2. Register the plugin in your Rons configuration:
```typescript
import { dcapPlugin } from "@rons-org/plugin-dcap";

// In your Rons configuration
plugins: [
    dcapPlugin,
    // ... other plugins
];
