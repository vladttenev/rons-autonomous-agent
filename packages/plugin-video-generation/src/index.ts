...
const generateVideo = async (prompt: string, runtime: IAgentRuntime) => {
    ...
    try {
        ronsLogger.log("Starting video generation with prompt:", prompt);
        ...
        if (!response.ok) {
            ...
            ronsLogger.error("Luma API error:", {
        ...
        while (status !== "completed" && status !== "failed") {
            ...
            if (!statusResponse.ok) {
                ...
                ronsLogger.error("Status check error:", {
            ...
            ronsLogger.log("Status check response:", statusData);
        ...
        ronsLogger.error("Video generation error:", error);
        return {
            success: false,
...
const videoGeneration: Action = {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.log("Validating video generation action");
        const lumaApiKey = runtime.getSetting("LUMA_API_KEY");
        ronsLogger.log("LUMA_API_KEY present:", !!lumaApiKey);
        return !!lumaApiKey;
    },
    handler: async (
        ...
        ronsLogger.log("Video generation request:", message);
        ...
        ronsLogger.log("Video prompt:", videoPrompt);
        ...
        try {
            ...
            ronsLogger.error(`Failed to generate video. Error: ${error}`);
            callback({
