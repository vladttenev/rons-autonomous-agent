# @rons-org/plugin-video-generation
...
```bash
npm install @rons-org/plugin-video-generation
```
...
Import and register the plugin in your Rons configuration:

```typescript
import { videoGenerationPlugin } from "@rons-org/plugin-video-generation";
...
```typescript
import { videoGeneration } from "@rons-org/plugin-video-generation";
...
- `@rons-org/core`: Core Rons functionality
- `tsup`: Build tool
