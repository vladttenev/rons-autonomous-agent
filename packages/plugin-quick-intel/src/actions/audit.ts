...
class TokenAuditAction {
    ...
    async audit(chain: string, tokenAddress: string): Promise<AuditResponse> {
        ronsLogger.log("Auditing token:", { chain, tokenAddress });
    ...
    async fetchDexData(tokenAddress: string, chain: string): Promise<DexResponse | null> {
        ronsLogger.log("Fetching DEX data:", { tokenAddress, chain });
        const myHeaders = new Headers();
        myHeaders.append("Content-Type", "application/json");
...
export const auditAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting QuickIntel audit handler...");
        ...
        try {
            ...
            // Perform audit
            ronsLogger.log("Performing audit for:", { chain, tokenAddress });
            ...
            ronsLogger.error("Error in AUDIT_TOKEN handler:", error?.message, error?.error);
