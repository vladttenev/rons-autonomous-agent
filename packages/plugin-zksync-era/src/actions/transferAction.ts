...
export const TransferAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting ZKsync Era SEND_TOKEN handler...");
        ...
        if (!isAddress(content.recipient, { strict: false })) {
            ronsLogger.log("Resolving ENS name...");
            try {
                ...
                if (isAddress(resolvedAddress, { strict: false })) {
                    ronsLogger.log(`${name} resolved to ${resolvedAddress}`);
                    content.recipient = resolvedAddress;
                }
            } catch (error) {
                ronsLogger.error("Error resolving ENS name:", error);
            }
        }
