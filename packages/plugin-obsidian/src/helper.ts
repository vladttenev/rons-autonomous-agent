import { type IAgentRuntime, type AgentRuntime, ModelClass, type Memory, MemoryManager, type State } from "@rons-org/core";
import { ronsLogger, composeContext, generateObject, stringToUuid } from "@rons-org/core";
...
export async function getObsidian(runtime: IAgentRuntime): Promise<ObsidianProvider> {
    if (!obsidianInstance) {
        ronsLogger.debug("Creating new ObsidianProvider instance");
...
export async function storeHierarchyInMemory(runtime: IAgentRuntime, message: Memory, hierarchy: NoteHierarchy) {
    ...
    ronsLogger.info(`Stored hierarchy for note ${hierarchy.path} in memory`);
}
...
export async function retrieveHierarchyFromMemory(runtime: IAgentRuntime, message: Memory, notePath: string): Promise<NoteHierarchy | null> {
    ...
    try {
        ...
        if (memories && memories.length > 0) {
            ...
            ronsLogger.info(`Retrieved hierarchy for note ${notePath} from memory`);
            return hierarchy;
        }
        return null;
    } catch (error) {
        ronsLogger.error(`Failed to retrieve hierarchy from memory: ${error.message}`);
        return null;
    }
}
...
async function genereteSearchParameters(
    ...
    try {
        ...
        ronsLogger.info("Parsed completion:", JSON.stringify(parsedCompletion, null, 2));
