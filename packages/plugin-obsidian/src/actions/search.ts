...
export const searchAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting search handler");
        const obsidian = await getObsidian(runtime);
        ...
        try {
            ...
            ronsLogger.debug("Search context:", JSON.stringify(searchContext.query, null, 2));
            ...
            ronsLogger.info(`Searching vault with ${queryFormat} query: ${typeof query === 'string' ? query : JSON.stringify(query)}`);
            ...
            ronsLogger.info(`Found ${results.length} matching notes`);
...
#### ✅ ${result.filename} (**Score:** ${result.score})\n${matches}`;
                ...
                ronsLogger.info("Formatted results:", formattedResults);
...
#### ✅ ${result.filename}`;
            ...
            ronsLogger.error("Error searching vault:", error);
            if (callback) {
                callback({
