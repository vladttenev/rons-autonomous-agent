...
export const listDirectoryAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting list directory handler");
        const obsidian = await getObsidian(runtime);
        ...
        try {
            ...
            ronsLogger.info(`Listing files in directory: ${directoryPath}`);
            const files: string[] = await obsidian.listDirectoryFiles(directoryPath);
            ronsLogger.info(`Successfully retrieved ${files.length} files`);
            ...
            ronsLogger.error("Error listing directory:", error);
            if (callback) {
                callback({
