...
export const getActiveNoteAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting get active note handler");
        const obsidian = await getObsidian(runtime);

        try {
            ronsLogger.info("Fetching active note content");
            ...
            ronsLogger.error("Error getting active note:", error);
            if (callback) {
                callback({
...
export const summarizeActiveNoteAction: Action = {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting summarize active note handler");
        const obsidian = await getObsidian(runtime);
        ...
        try {
            ...
            ronsLogger.info("Composing summary context");
            ...
            if (!currentSummary) {
                ronsLogger.error("Error: No summary found");
                return false;
            }
            if (callback) {
                if (
            ...
            ronsLogger.error("Error summarizing active note:", error);
            if (callback) {
                callback({
