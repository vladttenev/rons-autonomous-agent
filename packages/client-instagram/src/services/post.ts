...
export class InstagramPostService {
    ...
    private async generateNewPost() {
        try {
            ronsLogger.log("Generating new Instagram post");
            ...
            ronsLogger.error("Error generating Instagram post:", {
    ...
    private async getOrGenerateImage(content: string): Promise<string> {
        try {
            ronsLogger.log("Generating image for Instagram post");
...
seed: imageSettings?.seed || null,
      ...
      if (!cleanedContent) {
        ronsLogger.error("Failed to extract valid content from response:", {
          rawResponse: content,
          attempted: "JSON parsing",
        });
        return;
    ...
    async createPost(options: PostOptions) {
        ...
        try {
            ronsLogger.log("Creating Instagram post", {
            ...
            ronsLogger.log("Instagram post created successfully");
        } catch (error) {
            ronsLogger.error("Error creating Instagram post:", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                phase: "createPost",
                mediaCount: options.media.length,
    ...
    private async processMedia(media: {
        ...
        try {
            ronsLogger.log("Processing media", {
                type: media.type,
                url: media.url,
            });
            ...
            ronsLogger.error("Error processing media:", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                phase: "processMedia",
