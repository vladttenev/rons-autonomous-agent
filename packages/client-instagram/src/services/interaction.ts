  ...
  export class InstagramInteractionService {
    ...
    private async handleInteractions() {
      if (this.isProcessing) {
        ronsLogger.log("Already processing interactions, skipping");
        return;
      }

      try {
        this.isProcessing = true;
        ronsLogger.log("Checking Instagram interactions");
        ...
        ronsLogger.error("Error handling Instagram interactions:", error);
      } finally {
    ...
    private async handleComment(item: any) {
      try {
        ...
        if (!cleanedResponse) {
          ronsLogger.error("Failed to generate valid comment response");
          return;
        }
        ...
        ronsLogger.error("Error handling comment:", error);
      }
    }
      ...
      try {
        ...
        if (decision.includes("[INTERACT]")) {
          ...
          if (userFeed.length > 0) {
            ...
            ronsLogger.log(`Liked post from user: ${item.user?.username}`);
          }
        }
      } catch (error) {
        ronsLogger.error("Error handling like:", error);
      }
    }
    ...
    private async handleMention(item: any) {
      try {
        ...
        if (!cleanedResponse) {
          ronsLogger.error("Failed to generate valid mention response");
          return;
        }
        ...
        ronsLogger.error("Error handling mention:", error);
      }
    }
