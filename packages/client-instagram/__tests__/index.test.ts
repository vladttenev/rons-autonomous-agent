...
// Mock dependencies
vi.mock('@rons-org/core', async (importOriginal) => {
...
describe('InstagramClientInterface', () => {
  ...
  it('starts successfully with all services', async () => {
    ...
    expect(ronsLogger.log).toHaveBeenCalledWith('Instagram client configuration validated');
    expect(ronsLogger.log).toHaveBeenCalledWith('Instagram client initialized');
    expect(ronsLogger.log).toHaveBeenCalledWith('Instagram post service started');
    expect(ronsLogger.log).toHaveBeenCalledWith('Instagram interaction service started');
  });
  ...
  it('starts in dry-run mode', async () => {
    ...
    expect(result).toBeDefined();
    expect(ronsLogger.log).toHaveBeenCalledWith('Instagram client running in dry-run mode');
  ...
  it('handles errors during startup', async () => {
    ...
    expect(ronsLogger.error).toHaveBeenCalledWith('Failed to start Instagram client:', error);
  });

  it('stops gracefully', async () => {
    await InstagramClientInterface.stop(mockRuntime);
    expect(ronsLogger.log).toHaveBeenCalledWith('Stopping Instagram client services...');
  });
});
