import { type IAgentRuntime, ModelClass, Service, ServiceType, ronsLogger, generateObjectDeprecated, generateText } from "@rons-org/core";
...
export class GoplusSecurityService extends Service implements IGoplusSecurityService {
    ...
    async check(text: string): Promise<string> {
        try {
            ronsLogger.log("check input text", text);
            ...
            ronsLogger.log("check generateObjectDeprecated text", obj);
