...
export class EchoChamberClient {
    ...
    public async addWatchedRoom(roomId: string): Promise<void> {
        try {
            ...
            ronsLogger.success(`Now watching room: ${room.name}`);
        } catch (error) {
            ronsLogger.error("Error adding watched room:", error);
            throw error;
        }
    }
    ...
    public removeWatchedRoom(roomId: string): void {
        ...
        ronsLogger.success(`Stopped watching room: ${roomId}`);
    }
    ...
    private async retryOperation<T>(
        ...
        for (let i = 0; i < retries; i++) {
            try {
                ...
                ronsLogger.warn(`Retrying operation in ${delay}ms...`);
    ...
    public async start(): Promise<void> {
        ronsLogger.log("🚀 Starting EchoChamber client...");
        try {
            ...
            ronsLogger.error("❌ Failed to start EchoChamber client:", error);
            throw error;
        }
    }
    ...
    public async stop(): Promise<void> {
        ...
        ronsLogger.log("Stopping EchoChamber client...");
    }
    ...
    public async listRooms(tags?: string[]): Promise<ChatRoom[]> {
        try {
            ...
            ronsLogger.error("Error listing rooms:", error);
            throw error;
        }
    }
    ...
    public async shouldInitiateConversation(room: ChatRoom): Promise<boolean> {
        try {
            ...
            ronsLogger.error(`Error checking conversation state: ${error}`);
            return false;
        }
    }
}
