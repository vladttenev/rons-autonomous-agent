...
import { getActorDetails } from "@rons-org/core";
import {
    type Action,
    type ActionExample,
    type Content,
...
const summarizeAction: Action = {
    ...
    handler: async (
        ...
        if (!dateRange) {
            ronsLogger.error("Couldn't determine date range from message");
            callbackData.text =
...
Here is a detailed summary of the conversation between ${userName} and ${runtime.character.name}:\n\n${currentSummary.trim()}`;
            ...
            if (summaryContent.length > 1000) {
                ...
                ronsLogger.debug("Uploading summary file to Slack...");
                ...
                try {
                    ...
                    ronsLogger.error("Error uploading summary file:", error);
            ...
            ronsLogger.error("Error in summary generation:", error);
            callbackData.text =
