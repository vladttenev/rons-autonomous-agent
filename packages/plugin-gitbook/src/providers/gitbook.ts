...
async function validateQuery(
    ...
    try {
        ...
        ronsLogger.info(`✅ Is GitBook Validation Result: ${isValid}`);
        return isValid;
    } catch (error) {
        ronsLogger.warn(`❌ Error in GitBook validation:\n${error}`);
        return false;
    }
}
...
export const gitbookProvider: Provider = {
    get: async (
        ...
        try {
            ...
            if (!spaceId) {
                ronsLogger.error("⚠️ GitBook Space ID not configured");
                return "";
            }
            ...
            if (!isValidQuery) {
                ronsLogger.info("⚠️ GitBook Query validation failed");
                return "";
            }
            ...
            if (!response.ok) {
                ronsLogger.error("❌ GitBook API error:", response.status);
                return "";
            }
...
${result.answer?.text || ""}`;
            ...
            ronsLogger.error("❌ Error in GitBook provider:", error);
            return "";
        }
    },
};
