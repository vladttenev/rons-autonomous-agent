# @rons-org/plugin-gitbook

A plugin for querying and retrieving information from GitBook documentation within the RonsOS ecosystem.
...
```bash
pnpm install @rons-org/plugin-gitbook
```
...
```typescript
import { gitbookPlugin } from "@rons-org/plugin-gitbook";
```
