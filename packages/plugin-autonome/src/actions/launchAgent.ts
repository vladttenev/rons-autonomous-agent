...
function isLaunchAgentContent(content: any): content is LaunchAgentContent {
    ronsLogger.log("Content for launchAgent", content);
...
export default {
    name: "LAUNCH_AGENT",
    similes: ["CREATE_AGENT", "DEPLOY_AGENT", "DEPLOY_RONS", "DEPLOY_BOT"],
    validate: async (_runtime: IAgentRuntime, _message: Memory) => {
        return true;
    },
    description: "Launch an Rons agent",
    handler: async (
        ...
        ronsLogger.log("Starting LAUNCH_AGENT handler...");
        ...
        if (!isLaunchAgentContent(content)) {
            ronsLogger.error("Invalid launch content", content);
            if (callback) {
                callback({
        ...
        try {
            ...
            if (callback) {
                ronsLogger.error("Error during launching agent");
