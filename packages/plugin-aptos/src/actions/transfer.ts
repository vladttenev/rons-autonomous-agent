...
import { generateObjectDeprecated } from "@rons-org/core";
import {
    Account,
    Aptos,
    AptosConfig,
...
export default {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory) => {
        ronsLogger.log("Validating apt transfer from user:", message.userId);
        //add custom validate logic here
        /*
            const adminIds = runtime.getSetting("ADMIN_USER_IDS")?.split(",") || [];
            //ronsLogger.log("Admin IDs from settings:", adminIds);

            const isAdmin = adminIds.includes(message.userId);

            if (isAdmin) {
                //ronsLogger.log(`Authorized transfer from user: ${message.userId}`);
                return true;
            }
            else
            {
                //ronsLogger.log(`Unauthorized transfer attempt from user: ${message.userId}`);
                return false;
            }
            */
        return false;
    ...
    handler: async (
        ...
        ronsLogger.log("Starting SEND_TOKEN handler...");
        ...
        if (!isTransferContent(content)) {
            ronsLogger.error("Invalid content for TRANSFER_TOKEN action.");
            if (callback) {
                callback({
        ...
        try {
            ...
            ronsLogger.log("Transfer successful:", executedTransaction.hash);
            ...
            ronsLogger.error("Error during token transfer:", error);
            if (callback) {
                callback({
