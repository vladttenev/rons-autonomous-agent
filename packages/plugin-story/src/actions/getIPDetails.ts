...
class GetIPDetailsAction {
    async getIPDetails(
        ...
        ronsLogger.log("Fetching from", `${API_URL}/${RESOURCE_TYPE.ASSET}`);
...
export const getIPDetailsAction = {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting GET_IP_DETAILS handler...");
        ...
        try {
            ...
            ronsLogger.error("Error fetching IP details:", e.message);
            callback?.({
