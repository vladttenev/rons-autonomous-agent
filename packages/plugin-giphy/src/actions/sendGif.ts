...
export default {
    ...
    validate: async (runtime: IAgentRuntime, _message: Memory) => {
        ronsLogger.log("🔄 Validating Giphy configuration...");
        try {
    ...
    handler: async (
        ...
        ronsLogger.log("🚀 Starting Giphy SEND_GIF handler...");

        // Initialize or update state
        let currentState = state;
        if (!currentState) {
            ronsLogger.log("Creating new state...");
            currentState = (await runtime.composeState(message)) as State;
        } else {
            ronsLogger.log("Updating existing state...");
            currentState = await runtime.updateRecentMessageState(currentState);
        }
        try {
            ronsLogger.log("Composing gif trigger context...");
            const gifContext = composeContext({
                state: currentState,
                template: sendGifTemplate,
            });

            ronsLogger.log("Generating content from context...");
            ...
            if (!content.trigger || !content.searchTerm) {
                ronsLogger.log("No gif triggered for this message.");
                return false;
            }
            ...
            if (callback) {
                ...
                ronsLogger.log("✅ Sending callback with gif url:", message);
                callback(message);
            }
