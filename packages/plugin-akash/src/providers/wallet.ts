...
import { ronsLogger, type IAgentRuntime, type Memory } from "@rons-org/core";
// import { IAgentRuntime, Memory } from "@rons-org/core/src/types";
...
export const walletProvider: AkashProvider = {
    ...
    initialize: async (runtime: IAgentRuntime): Promise<void> => {
        ronsLogger.info("Initializing Akash wallet provider");
        try {
            ...
            ronsLogger.info("Akash wallet provider initialized successfully", {
                address,
            });
        } catch (error) {
            ronsLogger.error("Failed to initialize Akash wallet provider", {
