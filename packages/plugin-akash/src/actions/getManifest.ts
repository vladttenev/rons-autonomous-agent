...
// ronsLogger.info("Default SDL path initialized", { DEFAULT_SDL_PATH });
// ronsLogger.info("Loading SDL from file", { filePath });
// ronsLogger.info("Resolved SDL file path", { resolvedPath });
// ronsLogger.error("SDL file not found", { resolvedPath });
// ronsLogger.info("SDL file loaded successfully", { content });
// ronsLogger.error("Failed to read SDL file", { error });
// ronsLogger.error("SDL validation failed", { error });
// ronsLogger.info("Using provided SDL content");
// ronsLogger.info("Loading SDL from file", { path: params.sdlFile });
// ronsLogger.info("Loading default SDL", { path: DEFAULT_SDL_PATH });
// ronsLogger.debug("Parsing SDL content and generating manifest");
...
const validateSDL = (sdlContent: string, validationLevel = "strict"): boolean => {
    try {
        ...
        if (validationLevel === "none") {
            // ronsLogger.debug("Skipping SDL validation (validation level: none)");
            return true;
        }
        ...
        // ronsLogger.debug("SDL validation successful", {
        //     validationLevel,
        //     checkedSections: sectionsToCheck
        // });
        return true;
    } catch (error) {
        ronsLogger.error("SDL validation failed", {
            error: error instanceof Error ? error.message : String(error),
            validationLevel
        });
        return false;
...
export const getManifestAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("Validating manifest request", { message });
        try {
            ...
            ronsLogger.error("Manifest validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting manifest operation", { actionId });
        ...
        try {
            ...
            try {
                ...
                if (params.sdl) {
                    ...
                    ronsLogger.info("Loaded SDL from file", { path: params.sdlFile });
                } else {
                    const sdlPath = config.AKASH_SDL || DEFAULT_SDL_PATH;
                    sdlContent = loadSDLFromFile(sdlPath);
                    ronsLogger.info("Using SDL from environment", { path: sdlPath });
                }
                ...
                if (manifestMode === "validate_only") {
                    ronsLogger.info("Validation successful (validate_only mode)");
                    if (callback) {
                ...
                if (config.AKASH_MANIFEST_PATH) {
                    ...
                    ronsLogger.info("Manifest saved", { path: manifestPath });
                }
                ...
                ronsLogger.error("Manifest operation failed", {
