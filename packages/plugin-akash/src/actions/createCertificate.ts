...
function loadCertificate(): CertificatePem {
    ronsLogger.debug("Loading certificate from file", { path: CERTIFICATE_PATH });
    try {
...
async function setupClient(wallet: DirectSecp256k1HdWallet, rpcEndpoint: string): Promise<SigningStargateClient> {
    ...
    for (const endpoint of rpcEndpoints) {
        try {
            ...
            ronsLogger.debug("Client setup completed successfully", {
                endpoint,
            ...
            ronsLogger.warn("Failed to connect to RPC endpoint", {
                endpoint,
                error: error instanceof Error ? error.message : String(error),
...
export const createCertificateAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("=== Starting Certificate Validation ===");
        try {
            ...
            ronsLogger.debug("Certificate validation completed successfully");
            return true;
        } catch (error) {
            ronsLogger.error("Certificate validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("=== Starting Certificate Creation/Loading ===", { actionId });
        ...
        try {
            ...
            try {
                ...
                if (fs.existsSync(CERTIFICATE_PATH) && !params.overwrite) {
                    ronsLogger.info("Loading existing certificate");
                ...
                // Initialize wallet
                ronsLogger.info("Initializing wallet for certificate creation");
                ...
                ronsLogger.info("Generating new certificate");
                ...
                ronsLogger.error("Failed to create/load certificate", {
                    error: error instanceof Error ? error.message : String(error),
                    stack: error instanceof Error ? error.stack : undefined
                });
            ...
            ronsLogger.error("Certificate operation failed", {
                error: error instanceof Error ? error.message : String(error),
