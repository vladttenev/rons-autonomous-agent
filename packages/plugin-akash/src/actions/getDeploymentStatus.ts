...
export const getDeploymentStatusAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("Validating get deployment status request", { message });
        try {
            ...
            if (params.text && !params.dseq) {
                ...
                if (dseqMatch) {
                    ...
                    ronsLogger.debug("Extracted DSEQ from text", {
                        text: params.text,
            ...
            if (!params.dseq) {
                if (config.AKASH_DEP_STATUS === "dseq" && config.AKASH_DEP_DSEQ) {
                    ...
                    ronsLogger.info("DSEQ parameter is required when AKASH_DEP_STATUS is set to param_passed", {
                    ...
                    ronsLogger.info("No DSEQ provided and no valid environment configuration found", {
            ...
            ronsLogger.error("Get deployment status validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting deployment status request", { actionId });
        ...
        try {
            ...
            // Query deployment
            ronsLogger.info("Querying deployment status", {
                dseq: params.dseq,
            ...
            try {
                ...
                ronsLogger.info("Deployment status retrieved successfully", {
                    dseq: params.dseq,
                    state: status.state,
                    owner: status.owner,
                    actionId
                ...
                if (callback) {
                    ...
                    ronsLogger.info("=== Preparing callback response for deployment status ===", {
                        hasCallback: true,
                        actionId,
                        dseq: params.dseq
                    });
                ...
                if (dseqSource === "environment") {
                    ronsLogger.warn("Failed to query deployment from environment configuration", {
                        dseq: params.dseq,
                    ...
                    ronsLogger.error("Failed to query deployment", {
                        error: queryError instanceof Error ? queryError.message : String(queryError),
                        actionId
                    });
                    if (callback) {
            ...
            ronsLogger.error("Get deployment status request failed", {
