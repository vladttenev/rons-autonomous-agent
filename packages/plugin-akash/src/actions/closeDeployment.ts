...
async function initializeClient(runtime: IAgentRuntime) {
    ronsLogger.info("=== Initializing Client for Deployment Closure ===");
    ...
    ronsLogger.info("Client initialization complete", {
...
async function verifyDeploymentStatus(runtime: IAgentRuntime, dseq: string): Promise<boolean> {
    ronsLogger.info("Verifying deployment status", { dseq });
...
async function closeSingleDeployment(
    ...
    ronsLogger.info("Closing single deployment", { dseq });
    ...
    try {
        ...
        // Send transaction
        ronsLogger.info("Sending close deployment transaction", { dseq });
        ...
        ronsLogger.info("Deployment closed successfully", {
            dseq,
        ...
        ronsLogger.error("Failed to close deployment", {
            dseq,
...
async function closeAllDeployments(
    ...
    ronsLogger.info("Closing all active deployments");
    ...
    try {
        ...
        if (activeDeployments.length === 0) {
            ronsLogger.info("No active deployments found to close");
            return { success: [], failed: [] };
        }

        ronsLogger.info("Found active deployments to close", {
        ...
        ronsLogger.info("Finished closing deployments", results);
        return results;
    } catch (error) {
        ronsLogger.error("Failed to close deployments", {
            error: error instanceof Error ? error.message : String(error),
            code: error instanceof AkashError ? error.code : AkashErrorCode.DEPLOYMENT_CLOSE_FAILED,
            stack: error instanceof Error ? error.stack : undefined
        });
...
export const closeDeploymentAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("=== Starting Close Deployment Validation ===");
        ronsLogger.debug("Validating close deployment request", { message });
        ...
        try {
            ...
            const config = await validateAkashConfig(runtime);
            ronsLogger.debug("Validating parameters", { params });
            ...
            ronsLogger.error("Close deployment validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("=== Starting Close Deployment Request ===", {
            actionId,
        ...
        try {
            ...
            if (params.closeAll) {
                ...
                if (callback) {
                    ronsLogger.info("=== Preparing callback response for bulk closure ===", {
                        hasCallback: true,
                        actionId,
                ...
                if (callback) {
                    ronsLogger.info("=== Preparing callback response for single closure ===", {
                        hasCallback: true,
                        actionId,
                        dseq: params.dseq,
                        success
            ...
            ronsLogger.error("Close deployment request failed", {
                error: error instanceof Error ? error.message : String(error),
                code: error instanceof AkashError ? error.code : AkashErrorCode.DEPLOYMENT_CLOSE_FAILED,
                actionId
            });
