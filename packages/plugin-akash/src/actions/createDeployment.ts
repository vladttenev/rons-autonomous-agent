import { type Action, ronsLogger } from "@rons-org/core";
import type { IAgentRuntime, Memory, State, HandlerCallback, Content, ActionExample } from "@rons-org/core";
...
function saveCertificate(certificate: CertificatePem) {
    ronsLogger.debug("Saving certificate to file", { path: CERTIFICATE_PATH });
    try {
        ...
        ronsLogger.debug("Certificate saved successfully");
    } catch (error) {
        ronsLogger.error("Failed to save certificate", {
...
function loadCertificate(path: string): CertificatePem {
    ronsLogger.debug("Loading certificate from file", { path });
    try {
        const json = fs.readFileSync(path, "utf8");
        const certificate = JSON.parse(json);
        ronsLogger.debug("Certificate loaded successfully", {
        ...
        ronsLogger.error("Failed to load certificate", {
            error: error instanceof Error ? error.message : String(error),
            path
        });
        throw error;
...
const DEFAULT_SDL_PATH = (() => {
    ...
    // ronsLogger.info("=== Starting SDL Path Resolution in createDeployment ===", {
    //     currentFileUrl,
    //     cwd: process.cwd(),
    ...
    if (!fs.existsSync(sdlPath)) {
        ronsLogger.warn("Default SDL path not found", {
            sdlPath,
            exists: false
        });
    }
...
const loadSDLFromFile = (filePath: string): string => {
    // ronsLogger.info("=== Loading SDL File ===", {
    ...
    try {
        ...
        if (!filePath.includes('plugin-akash') && filePath !== DEFAULT_SDL_PATH) {
            ...
            // ronsLogger.info("Adjusting SDL path", {
        ...
        // ronsLogger.info("Attempting to load SDL from possible paths", {
        //     possiblePaths,
        ...
        for (const tryPath of possiblePaths) {
            if (fs.existsSync(tryPath)) {
                ...
                ronsLogger.info("SDL file loaded successfully from", {
                    path: tryPath
                });
                return content;
            }
        ...
        ronsLogger.error("Failed to read SDL file", {
            filePath,
            error: error instanceof Error ? error.message : String(error)
        });
...
async function initializeWallet(mnemonic: string) {
    ronsLogger.debug("=== Initializing Wallet ===", {
    ...
    if (words.length !== 12 && words.length !== 24) {
        ...
        ronsLogger.error("Mnemonic validation failed", {
            error,
    ...
    try {
        ronsLogger.debug("Creating wallet with mnemonic", {
            wordCount: words.length,
            mnemonicPreview: words.slice(0, 3).join(' ') + '...'
        });
        ...
        ronsLogger.debug("Wallet initialized successfully", {
        ...
        ronsLogger.error("Wallet initialization failed", {
            error: error instanceof Error ? error.message : String(error),
...
async function setupClient(wallet: DirectSecp256k1HdWallet, rpcEndpoint: string) {
    ...
    ronsLogger.info("=== Setting up Stargate Client ===", {
    ...
    for (const endpoint of rpcEndpoints) {
        try {
            ronsLogger.debug("Attempting to connect to RPC endpoint", {
                endpoint,
            ...
            ronsLogger.debug("Registry created for endpoint", {
                endpoint,
            ...
            try {
                ...
                ronsLogger.info("Stargate client setup successful", {
                    endpoint,
                    height,
                ...
                ronsLogger.error("Failed to get chain height", {
                    endpoint,
                    attempt: rpcEndpoints.indexOf(endpoint) + 1,
            ...
            ronsLogger.error("Failed to connect to RPC endpoint", {
                endpoint,
                attempt: rpcEndpoints.indexOf(endpoint) + 1,
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined
    ...
    ronsLogger.error("All RPC endpoints failed", {
...
async function fetchBid(dseq: number, owner: string, rpcEndpoint: string) {
    ronsLogger.info("=== Starting Bid Fetch Process ===", {
        dseq,
        owner,
    ...
    for (let retry = 0; retry < maxRetries; retry++) {
        try {
            ronsLogger.debug("Connecting to RPC for bid fetch", {
                rpcEndpoint,
                attempt: retry + 1,
                maxRetries
            });
            ...
            ronsLogger.debug("RPC connection established", {
            ...
            ronsLogger.debug("Starting bid polling loop", {
                timeout: "5 minutes",
            ...
            while (Date.now() - startTime < timeout) {
                ...
                ronsLogger.debug("Polling for bids", {
                    dseq,
                ...
                try {
                    ...
                    if (bids.bids.length > 0 && bids.bids[0].bid !== undefined) {
                        ronsLogger.info("Bid found successfully", {
                            dseq,
                            owner: owner.substring(0, 6),
                    ...
                    ronsLogger.warn("Temporary error during bid polling", {
            ...
            ronsLogger.error("Bid fetch timeout", {
                dseq,
                owner: owner.substring(0, 6),
                timeout: "5 minutes",
                attempt: retry + 1
            ...
            lastError = error instanceof Error ? error : new Error(String(error));
            ronsLogger.error("Error during bid fetch", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                dseq,
                owner: owner.substring(0, 6),
            ...
            if (retry < maxRetries - 1) {
                ...
                ronsLogger.info("Retrying bid fetch after delay", {
                    delay,
                    nextAttempt: retry + 2,
                    maxRetries
                });
    ...
    ronsLogger.error("All bid fetch attempts failed", {
        dseq,
        owner: owner.substring(0, 6),
        attempts: maxRetries,
...
async function createLease(deployment: any, wallet: DirectSecp256k1HdWallet, client: SigningStargateClient, rpcEndpoint: string): Promise<any> {
    ...
    ronsLogger.info("Starting lease creation", { dseq, owner });

    try {
        ronsLogger.debug("Fetching bid for lease creation");
        const bid = await fetchBid(dseq, owner, rpcEndpoint);
        const accounts = await wallet.getAccounts();

        if (bid.bidId === undefined) {
            ronsLogger.error("Invalid bid - missing bidId", { dseq, owner });
            throw new AkashError("Bid ID is undefined", AkashErrorCode.INVALID_BID);
        }

        ronsLogger.debug("Creating lease message", {
            dseq,
            owner,
            bidId: bid.bidId
        });
        ...
        ronsLogger.info("Broadcasting lease creation transaction");
        const tx = await client.signAndBroadcast(accounts[0].address, [msg], fee, "create lease");

        if (tx.code !== 0) {
            ronsLogger.error("Lease creation failed", {
                dseq,
                owner,
                code: tx.code,
                rawLog: tx.rawLog
        ...
        ronsLogger.info("Lease created successfully", {
            dseq,
            owner,
        ...
        ronsLogger.error("Error during lease creation", {
            error,
            dseq,
            owner
        });
...
async function queryLeaseStatus(lease: any, providerUri: string, certificate: CertificatePem): Promise<LeaseStatus | undefined> {
    const id = lease.id;
    ronsLogger.info("Querying lease status", {
        dseq: id?.dseq,
        gseq: id?.gseq,
        oseq: id?.oseq,
        providerUri
    ...
    if (id === undefined) {
        ronsLogger.error("Invalid lease - missing ID");
    ...
    ronsLogger.debug("Setting up request", {
        providerUri,
        leasePath,
        hasCert: !!certificate.cert,
    ...
    while (retryCount < MAX_RETRIES) {
        try {
            ...
            ronsLogger.debug("Making request", {
                url: fullUrl,
                method: 'GET',
            ...
            try {
                ...
                if (response.status !== 200) {
                    ronsLogger.warn("Non-OK response from lease status query", {
                    ...
                    if (response.status === 404) {
                        ronsLogger.debug("Deployment not ready yet (404)", {
                            dseq: id.dseq,
                            retryCount
                        });
                        return undefined;
                ...
                ronsLogger.debug("Lease status received", {
                    dseq: id.dseq,
            ...
            ronsLogger.warn("Error during lease status query", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                dseq: id.dseq,
                providerUri,
            ...
            if (retryCount < MAX_RETRIES - 1) {
                ...
                ronsLogger.debug("Retrying after error", {
                    delay,
            ...
            if (error instanceof Error &&
                ...
                ronsLogger.info("Returning undefined after max retries", {
                    dseq: id.dseq,
                    error: error.message
                });
                return undefined;
    ...
    ronsLogger.info("Max retries reached, returning undefined", {
        dseq: id.dseq,
        maxRetries: MAX_RETRIES
    });
    return undefined;
...
async function sendManifest(sdl: SDL, lease: any, certificate: CertificatePem, rpcEndpoint: string) {
    ronsLogger.info("Starting manifest send process");
    ...
    try {
        ...
        ronsLogger.debug("Getting provider info", { provider });
        ...
        if (tx.provider === undefined) {
            ronsLogger.error("Provider not found", { provider });
            throw new AkashError(
        ...
        ronsLogger.debug("Provider info retrieved", {
            provider,
        ...
        ronsLogger.info("Sending manifest to provider", {
            dseq,
            provider,
        ...
        try {
            ...
            ronsLogger.debug("Making manifest request", {
                url: fullUrl,
                method: 'PUT',
                manifestLength: manifest.length
            });
            ...
            if (response.status !== 200) {
                ronsLogger.error("Failed to send manifest", {
                    statusCode: response.status,
                    statusText: response.statusText,
                    dseq
                });
            ...
            ronsLogger.info("Manifest sent successfully", { dseq });
        } finally {
            httpsAgent.destroy();
        }

        // Wait for deployment to start
        ronsLogger.info("Waiting for deployment to start", { dseq });
        const startTime = Date.now();
        ...
        while (Date.now() - startTime < timeout) {
            const elapsedTime = Math.round((Date.now() - startTime) / 1000);
            ronsLogger.debug("Checking deployment status", {
                dseq,
            ...
            try {
                ...
                if (status === undefined) {
                    consecutiveErrors++;
                    ronsLogger.debug("Status check returned undefined", {
                        dseq,
                        consecutiveErrors,
                    ...
                    if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                        ronsLogger.warn("Too many consecutive undefined status responses", {
                            dseq,
                            consecutiveErrors
                        });
                ...
                for (const [name, service] of Object.entries<{ uris?: string[] }>(status.services)) {
                    if (service.uris) {
                        ...
                        ronsLogger.info("Service is available", {
                            name,
                            rawUrl,
                            serviceUrl,
                            dseq
                ...
                consecutiveErrors++;
                const errorMessage = error instanceof Error ? error.message : String(error);
                ronsLogger.warn("Error checking deployment status", {
                    error: errorMessage,
                    dseq,
                    consecutiveErrors,
                    maxConsecutiveErrors: MAX_CONSECUTIVE_ERRORS
                ...
                if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
                    ronsLogger.error("Too many consecutive errors checking deployment status", {
                        dseq,
                        consecutiveErrors,
                        error: errorMessage
                    });
        ...
        ronsLogger.error("Deployment start timeout", {
            dseq,
            timeout: "10 minutes"
        });
        throw new AkashError(
        ...
        ronsLogger.error("Error during manifest send process", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            dseq: lease.id.dseq
        });
...
async function loadOrCreateCertificate(wallet: DirectSecp256k1HdWallet, client: SigningStargateClient): Promise<CertificatePem> {
    ronsLogger.info("=== Starting Certificate Creation/Loading Process ===");
    try {
        const accounts = await wallet.getAccounts();
        const address = accounts[0].address;
        ronsLogger.debug("Got wallet address for certificate", {
            address,
        ...
        if (fs.existsSync(CERTIFICATE_PATH)) {
            ronsLogger.info("Found existing certificate file", { path: CERTIFICATE_PATH });
            const cert = loadCertificate(CERTIFICATE_PATH);
            ronsLogger.debug("Loaded existing certificate", {
                hasCert: !!cert.cert,
        ...
        ronsLogger.info("No existing certificate found, creating new one", { address });
        const certificate = certificateManager.generatePEM(address);
        ronsLogger.debug("Certificate generated", {
            hasCert: !!certificate.cert,
            hasPrivateKey: !!certificate.privateKey,
            hasPublicKey: !!certificate.publicKey,
        ...
        ronsLogger.info("Broadcasting certificate to network", {
            address,
            certLength: certificate.cert?.length,
            publicKeyLength: certificate.publicKey?.length
        });
        ...
        const result = await cert.broadcastCertificate(
            ...
            ronsLogger.error("Certificate broadcast failed", {
                error: error instanceof Error ? error.message : String(error),
                stack: error instanceof Error ? error.stack : undefined,
                address,
                certLength: certificate.cert?.length
        ...
        if (result.code !== 0) {
            ...
            ronsLogger.error("Certificate broadcast returned error code", {
                code: result.code,
        ...
        ronsLogger.info("Certificate broadcast successful", {
            code: result.code,
            txHash: result.transactionHash,
        ...
        ronsLogger.info("Certificate saved to file", { path: CERTIFICATE_PATH });

        ronsLogger.info("Certificate process completed successfully", {
            hasCert: !!certificate.cert,
            hasPrivateKey: !!certificate.privateKey,
            hasPublicKey: !!certificate.publicKey,
            path: CERTIFICATE_PATH
        ...
        ronsLogger.error("Certificate creation/broadcast process failed", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            path: CERTIFICATE_PATH
        });
...
async function parseSDL(sdlContent: string): Promise<SDL> {
    try {
        ...
        ronsLogger.info("Starting SDL parsing process", {
        ...
        ronsLogger.debug("Initial SDL parsing successful", {
        ...
        ronsLogger.info("SDL validation completed", {
        ...
        ronsLogger.error("Failed to parse SDL", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
...
export const createDeploymentAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("=== Starting Deployment Validation ===");
        ronsLogger.debug("Validating deployment request", { message });

        // Check if plugin is properly loaded
        if (!isPluginLoaded(runtime, "akash")) {
            ronsLogger.error("Akash plugin not properly loaded during validation");
            return false;
        }

        try {
            const params = message.content as Partial<CreateDeploymentContent>;
            ronsLogger.debug("Checking SDL content", { params });
            ...
            ronsLogger.debug("Validating SDL format");
            try {
                // Clean up SDL content by taking only the part after the YAML document separator
                const yamlSeparatorIndex = sdlContent.indexOf('---');
                if (yamlSeparatorIndex === -1) {
                ...
                ronsLogger.debug("SDL format validation successful", {
                    groups: sdl.groups(),
                    groupCount: sdl.groups().length
                });
            } catch (sdlError) {
                ronsLogger.error("SDL format validation failed", { error: sdlError });
                throw new AkashError(
            ...
            ronsLogger.debug("Validation completed successfully");
            return true;
        } catch (error) {
            ronsLogger.error("Deployment validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("=== Starting Deployment Creation ===", {
            actionId,
        ...
        try {
            ronsLogger.debug("=== Validating Akash Configuration ===");
            const config = await validateAkashConfig(runtime);
            ronsLogger.debug("Configuration validated successfully", {
            ...
            ronsLogger.debug("=== Processing Deployment Parameters ===", {
                hasSDL: !!params.sdl,
            ...
            ronsLogger.debug("SDL content loaded", {
                source: sdlSource,
                contentLength: sdlContent.length
            });

            if (params.deposit && !validateDeposit(params.deposit)) {
                ronsLogger.error("Invalid deposit format", {
            ...
            ronsLogger.info("=== Initializing Wallet and Client ===");
            const wallet = await initializeWallet(config.AKASH_MNEMONIC);
            const accounts = await wallet.getAccounts();
            const address = accounts[0].address;
            ronsLogger.debug("Wallet initialized", {
                address,
                accountCount: accounts.length
            });

            // Setup client
            ronsLogger.debug("Setting up Stargate client");
            const client = await setupClient(wallet, config.RPC_ENDPOINT);
            ronsLogger.debug("Client setup completed", {
                rpcEndpoint: config.RPC_ENDPOINT
            });

            // Load or create certificate
            ronsLogger.info("=== Setting up Certificate ===");
            const certificate = await loadOrCreateCertificate(wallet, client);
            ronsLogger.debug("Certificate setup completed", {
                hasCert: !!certificate.cert,
                hasPrivateKey: !!certificate.privateKey,
                hasPublicKey: !!certificate.publicKey
            });
            ...
            // Parse SDL
            ronsLogger.info("=== Parsing SDL Configuration ===");
            let sdl: SDL;
            try {
                sdl = await parseSDL(sdlContent);
                ronsLogger.debug("SDL parsed successfully", {
                    groupCount: sdl.groups().length,
                    groups: sdl.groups(),
                    version: await sdl.manifestVersion()
                });
            } catch (sdlError) {
                ronsLogger.error("SDL parsing failed", {
            ...
            ronsLogger.info("=== Creating Deployment Message ===");
            const blockHeight = await client.getHeight();
            ronsLogger.debug("Current block height", { blockHeight });
            ...
            ronsLogger.debug("Deployment object created", {
            ...
            ronsLogger.info("=== Broadcasting Deployment Transaction ===", {
                owner: address,
                dseq: blockHeight,
            ...
            const result = await withRetry(async () => {
                ronsLogger.debug("Attempting to sign and broadcast transaction", {
                    attempt: 'current',
                ...
                ronsLogger.debug("Transaction broadcast result", {
                    code: txResult.code,
                ...
                if (txResult.code !== 0) {
                    ronsLogger.error("Transaction failed", {
                        code: txResult.code,
                        rawLog: txResult.rawLog,
                        groups: deployment.groups
                    });
            ...
            ronsLogger.info("=== Deployment Created Successfully ===", {
                txHash: result.transactionHash,
                owner: address,
                dseq: blockHeight,
                actionId,
            ...
            // Create lease
            ronsLogger.debug("=== Creating Lease ===");
            const lease = await createLease(deployment, wallet, client, config.RPC_ENDPOINT);
            ronsLogger.debug("Lease created", {
                leaseId: lease.id,
                dseq: deployment.id.dseq
            });

            // Send manifest
            ronsLogger.debug("=== Sending Manifest ===");
            const serviceUrl = await sendManifest(sdl, lease, certificate, config.RPC_ENDPOINT);
            ronsLogger.debug("Manifest sent successfully", {
                serviceUrl
            });

            if (callback) {
                ronsLogger.info("=== Preparing callback response for deployment creation ===", {
                    hasCallback: true,
                    actionId,
                ...
                ronsLogger.info("=== Executing callback with response ===", {
                    actionId,
                ...
                ronsLogger.info("=== Callback executed successfully ===", {
                    actionId,
                    timestamp: new Date().toISOString()
                });
            }

            ronsLogger.info("=== Deployment Process Completed Successfully ===", {
                actionId,
                txHash: result.transactionHash,
                dseq: blockHeight
            });
            ...
            ronsLogger.error("=== Deployment Creation Failed ===", {
                error: error instanceof AkashError ? {
                    category: error.category,
                    code: error.code,
                    message: error.message,
            ...
            if (callback) {
                ronsLogger.info("=== Preparing error callback response ===", {
                    actionId,
                    hasCallback: true,
                ...
                ronsLogger.info("=== Executing error callback ===", {
                    actionId,
                ...
                ronsLogger.info("=== Error callback executed successfully ===", {
                    actionId,
                    timestamp: new Date().toISOString()
                });
            }
