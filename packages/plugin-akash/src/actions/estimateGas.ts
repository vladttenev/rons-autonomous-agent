...
function getTotalTokensFromString(str: string): number {
    try {
        ...
        ronsLogger.warn("Failed to count tokens", { error });
        return 0;
    }
}
...
export const estimateGas: Action = {
  ...
  validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
    ronsLogger.debug("Validating gas estimation request", { message });
    try {
      ...
      if (!params.operation) {
        if (config.AKASH_GAS_OPERATION) {
          ...
          ronsLogger.info("Using operation from environment", { operation: params.operation });
        } else {
      ...
      if (params.operation === "close") {
        if (!params.dseq) {
          if (config.AKASH_GAS_DSEQ) {
            ...
            ronsLogger.info("Using DSEQ from environment", { dseq: params.dseq });
          } else {
            throw new AkashError(
      ...
      ronsLogger.error("Gas estimation validation failed", {
  ...
  handler: async (
    ...
    ronsLogger.info("Starting gas estimation", { actionId });

    ronsLogger.debug("=== Handler Parameters ===", {
        hasRuntime: !!runtime,
        hasMessage: !!message,
        hasState: !!state,
        hasOptions: !!options,
    ...
    try {
        ...
        // Estimate gas
        ronsLogger.info("Estimating gas for operation", {
        ...
        ronsLogger.info("Gas estimation completed", {
            gasEstimate,
            operation: params.operation,
            dseq: params.dseq,
            owner: account.address,
        ...
        if (callback) {
            ronsLogger.info("=== Preparing callback response for gas estimation ===", {
                hasCallback: true,
                actionId,
                operation: params.operation,
                dseq: params.dseq
            ...
            ronsLogger.warn("=== No callback provided for gas estimation ===", {
                actionId,
                operation: params.operation,
                dseq: params.dseq
            });
        ...
        ronsLogger.error("Gas estimation failed", {
        ...
        if (callback) {
            ...
            ronsLogger.warn("=== No callback provided for error handling ===", {
                actionId,
