...
export const getProviderInfoAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("Validating get provider info request", { message });
        try {
            ...
            if (params.text && !params.provider) {
                ...
                if (providerMatch) {
                    ...
                    ronsLogger.debug("Extracted provider address from text", {
                        text: params.text,
            ...
            ronsLogger.error("Get provider info validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting provider info request", { actionId });
        ...
        try {
            ...
            ronsLogger.info("Querying provider information", {
            ...
            try {
                ...
                ronsLogger.info("Querying provider status", {
                    hostUri: response.provider.hostUri,
                    actionId
                });

                const hostUri = response.provider.hostUri.replace(/^https?:\/\//, '');
                ronsLogger.debug("Making provider status request", { url: `https://${hostUri}/status` });
                ...
                try {
                    ...
                    if (!statusResponse.ok) {
                        ronsLogger.debug("Provider status not available", {
                    ...
                    ronsLogger.debug("Provider status fetch failed", {
                ...
                ronsLogger.info("Provider information retrieved successfully", {
                    provider: params.provider,
                    hostUri: response.provider.hostUri,
                ...
                if (callback) {
                    ronsLogger.info("=== Preparing callback response for provider info ===", {
                        hasCallback: true,
                        actionId,
                        provider: params.provider
                    });
            ...
            ronsLogger.error("Get provider info request failed", {
