...
async function fetchWithRetry(url: string, options: RequestInit, retries = 3, delay = 1000): Promise<Response> {
    for (let i = 0; i < retries; i++) {
        try {
            ...
            ronsLogger.warn(`API request failed (attempt ${i + 1}/${retries})`, {
            ...
            ronsLogger.warn(`API request error (attempt ${i + 1}/${retries})`, {
...
export async function initializeWallet(runtime: IAgentRuntime): Promise<{wallet: DirectSecp256k1HdWallet | null, address: string}> {
    try {
        ...
        ronsLogger.info("Initializing wallet with config", {
        ...
        if (config.AKASH_WALLET_ADDRESS) {
            ronsLogger.info("Using provided wallet address", {
        ...
        try {
            ronsLogger.info("Creating wallet from mnemonic");
            ...
            ronsLogger.info("Wallet initialized from mnemonic", {
                address,
...
export async function fetchDeployments(
    ...
    ronsLogger.info("Initializing deployment fetch", {
        status: status || 'all',
        skip,
        limit
    });
    ...
    try {
        ...
        ronsLogger.info("Fetching deployments from API", {
            address,
            status: status || 'all',
            skip,
            limit
        ...
        ronsLogger.debug("Making API request", { url });
        ...
        ronsLogger.info("Deployments fetched successfully", {
            count: data.count,
        ...
        ronsLogger.info("Using deployments directory", { deploymentDir });

        // Create base deployments directory if it doesn't exist
        if (!fs.existsSync(deploymentDir)) {
            ronsLogger.info("Creating deployments directory", { deploymentDir });
        ...
        for (const [status, deployments] of Object.entries(deploymentsByStatus)) {
            ...
            ronsLogger.info("Processing status directory", { statusDir, status, deploymentCount: deployments.length });

            // Ensure status directory exists
            if (!fs.existsSync(statusDir)) {
                ronsLogger.info("Creating status directory", { statusDir });
            ...
            await Promise.all(deployments.map(async (deployment) => {
                ...
                ronsLogger.debug("Saving deployment file", { filePath, dseq: deployment.dseq });
        ...
        ronsLogger.error("Failed to fetch deployments", {
            error: error instanceof Error ? error.message : String(error),
...
export async function saveDeploymentInfo(deploymentInfo: DeploymentInfo, filePath: string): Promise<void> {
    ronsLogger.info("Saving deployment info", {
    ...
    try {
        ...
        ronsLogger.debug("Deployment info saved successfully");
    } catch (error) {
        ronsLogger.error("Failed to save deployment info", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            filePath
        });
...
export async function loadDeploymentInfo(filePath: string): Promise<DeploymentInfo> {
    ronsLogger.info("Loading deployment info", { filePath });
    ...
    try {
        ...
        ronsLogger.debug("Deployment info loaded successfully", {
            dseq: deploymentInfo.dseq,
            owner: deploymentInfo.owner
        });
        ...
        ronsLogger.error("Failed to load deployment info", {
            error: error instanceof Error ? error.message : String(error),
            stack: error instanceof Error ? error.stack : undefined,
            filePath
        });
...
export const getDeploymentApiAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory): Promise<boolean> => {
        ronsLogger.debug("Validating get deployments request", { message });
        try {
            ...
            ronsLogger.error("Get deployments validation failed", {
    ...
    handler: async (
        ...
        ronsLogger.info("Starting deployment API request", { actionId });
        ...
        try {
            ...
            if (callback) {
                ronsLogger.info("=== Preparing callback response for deployments ===", {
                    hasCallback: true,
                    actionId,
            ...
            ronsLogger.error("Get deployments request failed", {
                error: error instanceof Error ? error.message : String(error),
                actionId
            });
            ...
            if (callback) {
                ...
                ronsLogger.info("=== Error callback executed ===", {
                    actionId,
                    timestamp: new Date().toISOString()
                });
            }
