...
export function inspectRuntime(runtime: IAgentRuntime) {
    ...
    if (hasPlugins) {
        ...
        ronsLogger.info("Akash plugin status:", {
    ...
    if (hasActions) {
        ...
        ronsLogger.info("Akash actions status:", {
...
export function isPluginLoaded(runtime: IAgentRuntime, pluginName: string): boolean {
    ...
    const plugins = (runtime as any).plugins as Plugin[];
    if (!plugins) {
        ronsLogger.warn(`No plugins array found in runtime`);
        return false;
    }
    ...
    if (!plugin) {
        ronsLogger.warn(`Plugin ${pluginName} not found in runtime plugins`);
        return false;
    }
    ...
    if (!actions || !actions.length) {
        ronsLogger.warn(`No actions found in runtime`);
        return false;
    }
    ...
    if (!registeredActions) {
        ronsLogger.warn(`Not all ${pluginName} actions are registered in runtime`);
        return false;
    }

    ronsLogger.info(`Plugin ${pluginName} is properly loaded and registered`);
    return true;
}