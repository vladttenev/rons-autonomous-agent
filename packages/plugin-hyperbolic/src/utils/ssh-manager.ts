...
export class SSHManager {
  ...
  static fromConfig(
    ...
    if (!config.HYPERBOLIC_SSH_PRIVATE_KEY_PATH) {
      ronsLogger.error('SSH private key path not configured');
  ...
  async connect(): Promise<void> {
    try {
      ...
      ronsLogger.debug('Initiating SSH connection', {
        host: config.host,
      ...
      await new Promise<void>((resolve, reject) => {
        this.client
          .on('ready', () => {
            ronsLogger.debug('SSH connection established');
            resolve();
          })
          .on('error', (err: Error) => {
            ronsLogger.error('SSH connection failed', { error: err.message });
            reject(err);
          })
          .connect(config);
      });
    } catch (error) {
      ronsLogger.error('Failed to establish SSH connection', {
  ...
  async executeCommand(command: string): Promise<CommandResult> {
    ...
    try {
      ronsLogger.debug('Executing SSH command', { command });

      return await new Promise<CommandResult>((resolve, reject) => {
        this.client.exec(command, (err, stream) => {
          if (err) {
            ronsLogger.error('Failed to execute command', {
              error: err.message,
              command
            });
            reject(err);
          ...
          stream.on('close', () => {
            ronsLogger.debug('Command execution completed', {
              command,
              exitCode
            });
            resolve({
      ...
      ronsLogger.error('Command execution failed', {
        error: error instanceof Error ? error.message : String(error),
        command
      });
      throw error;
  ...
  disconnect(): void {
    ...
    if (this.client) {
      ronsLogger.debug('Closing SSH connection');
      this.client.end();
    }
  }
