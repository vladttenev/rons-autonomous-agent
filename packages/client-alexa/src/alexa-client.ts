import { ronsLogger, IAgentRuntime } from "@rons-org/core";
...
export class AlexaClient {
    ...
    constructor(runtime: IAgentRuntime) {
        ronsLogger.log("📱 Constructing new AlexaClient...");
    ...
    public async start(): Promise<void> {
        ronsLogger.log("🚀 Starting Alexa bot...");
        try {
            await this.initializeBot();
        } catch (error) {
            ronsLogger.error("❌ Failed to launch Alexa bot:", error);
            throw error;
        }
    }
    ...
    private async initializeBot(): Promise<void> {
        ...
        ronsLogger.log("✨ Alexa bot successfully launched and is running!");
    ...
    async sendProactiveEvent(access_token: string): Promise<void> {
        ...
        try {
            ...
            switch (response.status) {
                case 202:
                    ronsLogger.log("✅ Proactive event sent successfully.");
                    break;
                case 400:
                    ronsLogger.error(
