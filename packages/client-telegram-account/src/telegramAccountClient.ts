...
export class TelegramAccountClient {
    ...
    constructor(runtime: IAgentRuntime, telegramAccountConfig: TelegramAccountConfig) {
        ronsLogger.log("📱 Constructing new TelegramAccountClient...");
    ...
    public async start(): Promise<void> {
        ronsLogger.log("🚀 Starting Telegram account...");

        try {
            await this.initializeAccount();
            this.setupEventsHandlers();

            ronsLogger.success(`✅ Telegram account client successfully started for character ${this.runtime.character.name}`);
        } catch (error) {
            ronsLogger.error("❌ Failed to launch Telegram account:", error);
            throw error;
        }
    }
    ...
    private newMessageHandler() {
        this.client.addEventHandler(async (event: NewMessageEvent) => {
            try {
                ...
                ronsLogger.error("Error sending message:", error);
            }
