...
export class TrustScoreManager {
    ...
    async getRecommenderBalance(recommenderWallet: string): Promise<number> {
        try {
            ...
            ronsLogger.error("Error fetching balance", error);
            return 0;
        }
    }
    ...
    async createTradeInBe(
        ...
        for (let attempt = 1; attempt <= retries; attempt++) {
            try {
                ...
                if (attempt < retries) {
                    ronsLogger.log(`Retrying in ${delayMs} ms...`);
                    await this.delay(delayMs); // Wait for the specified delay before retrying
                } else {
                    ronsLogger.error("All attempts failed.");
                }
            }
        }
    }
...
export const trustScoreProvider: Provider = {
    async get(
        ...
        try {
            ...
            if (!userId) {
                ronsLogger.error("User ID is missing from the message");
                return "";
            }
            ...
            ronsLogger.error("Error in trust score provider:", error.message);
