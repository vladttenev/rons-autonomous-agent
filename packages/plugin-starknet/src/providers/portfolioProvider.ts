...
export class WalletProvider {
    ...
    async getWalletPortfolio(): Promise<TokenBalances> {
        ...
        if (cachedValues) {
            ronsLogger.debug("Using cached data for getWalletPortfolio()");
            return cachedValues;
        }
    ...
    async getTokenUsdValues(): Promise<CoingeckoPrices> {
        ...
        if (cachedValues) {
            ronsLogger.debug("Using cached data for getTokenUsdValues()");
            return cachedValues;
        }
...
const walletProvider: Provider = {
    get: async (
        ...
        try {
            ...
            ronsLogger.error("Error in walletProvider.get():", error);
