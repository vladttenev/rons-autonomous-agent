...
export default {
    ...
    handler: async (
        ...
        ronsLogger.log("Starting CREATE_SUBDOMAIN handler...");
        ...
        if (!isSubdomainCreation(content)) {
            ronsLogger.error("Invalid content for CREATE_SUBDOMAIN action.");
            if (callback) {
                callback({
        ...
        try {
            ...
            ronsLogger.error("Error during subdomain transfer:", error);
            if (callback) {
                callback({
