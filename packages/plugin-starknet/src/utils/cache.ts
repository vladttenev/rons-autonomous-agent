...
export class Cache {
    ...
    constructor() {
        ...
        // Find the 'rons' folder in the filepath and adjust the cache directory path
        const ronsIndex = __dirname.indexOf("rons");
        if (ronsIndex !== -1) {
            const pathToRons = __dirname.slice(0, ronsIndex + 5); // include 'rons'
            this.cacheDir = path.join(pathToRons, "cache");
        } else {
