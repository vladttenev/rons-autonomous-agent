...
async function handleStreamResponse(
    ...
    ronsLogger.log("Starting stream response handling");
    const reader = response.body?.getReader();
    if (!reader) {
        ronsLogger.error("No readable stream available");
        ...
        async start(controller) {
            try {
                while (true) {
                    ...
                    if (done) {
                        ronsLogger.log("Stream reading completed");
                        break;
                    }
                ...
                reader.releaseLock();
                controller.close();
                ronsLogger.log("Stream controller closed");
            }
        },
    });
}
...
export const blockchainChatAction: Action = {
    ...
    handler: async (
        ...
        try {
            ronsLogger.log("Starting blockchain chat handler");
            const secretKey =
                runtime.getSetting("THIRDWEB_SECRET_KEY") ??
                process.env.THIRDWEB_SECRET_KEY;

            if (!secretKey) {
                ronsLogger.error("THIRDWEB_SECRET_KEY not configured");
            ...
            ronsLogger.log("NEBULA CHAT REQUEST: ", request);

            ronsLogger.debug("Sending request to Nebula API");
            ...
            ronsLogger.debug("Received response from Nebula API");

            if (!request.stream) {
                const text = await response.text();
                ronsLogger.debug("Raw response text:", text);

                try {
                    const cleanedText = text.trim().split("\n").pop() || text;
                    const parsed = JSON.parse(cleanedText);
                    ronsLogger.log("Successfully parsed response:", parsed);
                    ...
                    ronsLogger.error("Parse error details:", parseError);
                    ronsLogger.error(
            ...
            ronsLogger.log("Handling streaming response");
            return handleStreamResponse(response);
        } catch (error) {
            ronsLogger.error("Blockchain chat failed:", error);
