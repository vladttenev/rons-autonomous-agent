# @rons-org/plugin-omniflix

A plugin for RonsOS that enables interaction with the OmniFlix Network blockchain.
...
```bash
npm install @rons-org/plugin-omniflix
```
...
```typescript
import { OmniflixPlugin } from "@rons-org/plugin-omniflix";
```

2. Register with RonsOS:

```typescript
import { Rons } from "@rons-org/core";

const rons = new Rons();
rons.registerPlugin(OmniflixPlugin);
```

### Example Usage

```typescript
import { voteOnProposal } from "@rons-org/plugin-omniflix";
...
This plugin is part of the RonsOS project. See LICENSE file for details.
