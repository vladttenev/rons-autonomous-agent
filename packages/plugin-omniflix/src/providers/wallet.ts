import { ronsLogger, IAgentRuntime, Provider } from "@rons-org/core";
import {
...
export const walletProvider: Provider = {
    get: async (runtime: IAgentRuntime) => {
        try {
            ...
            if (!rpcEndpoint) {
                ronsLogger.error("RPC endpoint not found");
                return null;
            }

            if (!privateKey && !mnemonic) {
                ronsLogger.error("Neither private key nor mnemonic provided");
                return null;
            }
            ...
            if (privateKey) {
                ...
                ronsLogger.info("Wallet initialized with private key");
            } else if (mnemonic) {
                // Use mnemonic
                wallet = await DirectSecp256k1HdWallet.fromMnemonic(mnemonic, {
                    prefix: "omniflix",
                });
                ronsLogger.info("Wallet initialized with mnemonic");
            } else {
            ...
            ronsLogger.error(`Error initializing wallet: ${error.message}`);
            return null;
        }
    },
};
