...
import { DatabaseAdapter } from "@rons-org/core";
...
export class SupabaseDatabaseAdapter extends DatabaseAdapter {
    async getRoom(roomId: UUID): Promise<UUID | null> {
        ...
        if (error) {
            ronsLogger.error(`Error getting room: ${error.message}`);
            return null;
        }
    ...
    async getParticipantUserState(
        ...
        if (error) {
            ronsLogger.error("Error getting participant user state:", error);
            return null;
        }
    ...
    async setParticipantUserState(
        ...
        if (error) {
            ronsLogger.error("Error setting participant user state:", error);
    ...
    async getMemoriesByRoomIds(params: {
        ...
        if (error) {
            ronsLogger.error("Error retrieving memories by room IDs:", error);
            return [];
        }
    ...
    async createAccount(account: Account): Promise<boolean> {
        const { error } = await this.supabase
            ...
            .upsert([account]);
        if (error) {
            ronsLogger.error(error.message);
            return false;
        }
        return true;
    }
    ...
    async getActorDetails(params: { roomId: UUID }): Promise<Actor[]> {
      ...
      `
            ...
            if (response.error) {
                ronsLogger.error("Error!" + response.error);
                return [];
            }
            const { data } = response;
    ...
    async log(params: {
        ...
        if (error) {
            ronsLogger.error("Error inserting log:", error);
            throw new Error(error.message);
        }
    }
    ...
    async getMemoryById(memoryId: UUID): Promise<Memory | null> {
        ...
        if (error) {
            ronsLogger.error("Error retrieving memory by ID:", error);
            return null;
        }
    ...
    async addParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
        ...
        if (error) {
            ronsLogger.error(`Error adding participant: ${error.message}`);
            return false;
        }
        return true;
    }
    ...
    async removeParticipant(userId: UUID, roomId: UUID): Promise<boolean> {
        ...
        if (error) {
            ronsLogger.error(`Error removing participant: ${error.message}`);
            return false;
        }
        return true;
    }
    ...
    async getCache(params: {
        ...
        if (error) {
            ronsLogger.error("Error fetching cache:", error);
            return undefined;
        }
    ...
    async setCache(params: {
        ...
        if (error) {
            ronsLogger.error("Error setting cache:", error);
            return false;
        }
    ...
    async createKnowledge(knowledge: RAGKnowledgeItem): Promise<void> {
        try {
            ...
            ronsLogger.error(`Error creating knowledge ${knowledge.id}:`, {
                error,
