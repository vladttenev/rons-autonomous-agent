...
grep 'const importedPlugin = await import("@rons-org/plugin-ton");' agent/src/index.ts
if [ $? -eq 0 ]; then
    ...
    const importedPlugin = await import("@rons-org/plugin-ton");\
...
# add below to the root package.json to fix https://github.com/ronsOS/rons/issues/1965
