...
Internet Computer Protocol (ICP) plugin for Rons OS.
...
```bash
pnpm install @rons-org/plugin-icp
```
...
```typescript
import { icpPlugin } from "@rons-org/plugin-icp";

// Register the plugin with Rons
rons.registerPlugin(icpPlugin);
```
