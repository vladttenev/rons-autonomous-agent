# @rons-org/plugin-cronoszkevm

A plugin for interacting with the Cronos zkEVM network within the RonsOS ecosystem.
...
```bash
pnpm install @rons-org/plugin-cronoszkevm
```
...
```typescript
import { cronosZkEVMPlugin } from "@rons-org/plugin-cronoszkevm";
```
