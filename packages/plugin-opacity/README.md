# @rons-org/adapter-opacity

This adapter integrates Opacity proofs into RonsOS, enabling verifiable inference results from various AI model providers available through the [CloudFlare AI Gateway](https://developers.cloudflare.com/ai-gateway). It implements the `IVerifiableInferenceAdapter` interface, making it compatible with other verifiable inference solutions.

## Installation

```bash
pnpm add @rons-org/adapter-opacity
```
...
```typescript
import { OpacityAdapter } from "@rons-org/adapter-opacity";
import { VerifiableInferenceOptions } from "@rons-org/core";
