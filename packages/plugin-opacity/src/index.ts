...
export class OpacityAdapter implements IVerifiableInferenceAdapter {
    ...
    async generateText(
        ...
        ronsLogger.log("Generating text with options:", {
        ...
        try {
            ...
            ronsLogger.debug("Request body:", JSON.stringify(body, null, 2));
            ...
            ronsLogger.debug("Making request to Cloudflare with:", {
                endpoint,
                headers: {
                    ...requestHeaders,
            ...
            try {
                ...
                ronsLogger.error("Invalid JSON body:", body);
                throw new Error("Failed to create valid JSON request body");
            }
            ronsLogger.debug("Request body:", requestBody);
            ...
            if (!cloudflareResponse.ok) {
                ...
                ronsLogger.error("Cloudflare error response:", {
            ...
            ronsLogger.debug("Cloudflare response:", {
                status: cloudflareResponse.status,
                statusText: cloudflareResponse.statusText,
    ...
    async generateProof(baseUrl: string, logId: string) {
        ...
        ronsLogger.debug("Fetching proof for log ID:", logId);
        if (!response.ok) {
