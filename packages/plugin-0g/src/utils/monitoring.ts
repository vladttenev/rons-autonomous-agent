...
export const logSecurityEvent = (
    ...
    ronsLogger.info('Security event', securityEvent);

    // For high severity events, also log as error
    if (severity === 'high') {
        ronsLogger.error('High severity security event', securityEvent);
    }
};
...
export const monitorUpload = (metrics: Omit<UploadMetrics, 'timestamp'>): void => {
    ...
    ronsLogger.info('Upload metrics', uploadMetrics);

    // Log errors if present
    if (!metrics.success && metrics.error) {
        ronsLogger.error('Upload failed', {
