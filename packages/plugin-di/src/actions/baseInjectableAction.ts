...
export abstract class BaseInjectableAction<T> implements InjectableAction<T> {
     ...
     * @param callback The callback function to pass the result to Rons runtime
     */
    abstract execute(
        content: T | null,
    ...
    // -------- Implemented methods for Rons runtime --------

    /**
     * Default implementation of the validate method
     * You can override this method to add custom validation logic
     *
     * @param runtime The runtime object from Rons framework
     * @param message The message object from Rons framework
     * @param state The state object from Rons framework
    ...
    protected async processMessages(
        ...
        if (!actionContext) {
            ronsLogger.error("Failed to prepare action context");
            return null;
        }
        ...
        ronsLogger.debug("Response: ", resourceDetails.object);
     ...
     * @param message The message object from Rons framework
     * @param state The state object from Rons framework
     * @param options The options object from Rons framework
     * @param callback The callback function to pass the result to Rons runtime
     */
    async handler(
        runtime: IAgentRuntime,
        ...
        try {
            ...
            ronsLogger.error("Error in executing action:", err.message);
        }
    }
}
