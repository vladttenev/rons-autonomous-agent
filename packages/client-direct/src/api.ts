...
import type { TeeLogQuery, TeeLogService } from "@rons-org/plugin-tee-log";
import { REST, Routes } from "discord.js";
import type { DirectClient } from ".";
import { validateUuid } from "@rons-org/core";
...
export function createApiRouter(
    ...
    router.post("/agents/:agentId/set", async (req, res) => {
        ...
        try {
            ...
            ronsLogger.error(`Error parsing character: ${e}`);
            res.status(400).json({
                success: false,
                message: e.message,
            });
        ...
        try {
            ...
            ronsLogger.log(`${character.name} started`);
        } catch (e) {
            ronsLogger.error(`Error starting agent: ${e}`);
            res.status(500).json({
                success: false,
                message: e.message,
            });
    ...
    router.get("/tee/agents", async (req, res) => {
        try {
            ...
            ronsLogger.error("Failed to get TEE agents:", error);
            res.status(500).json({
    ...
    router.get("/tee/agents/:agentId", async (req, res) => {
        try {
            ...
            ronsLogger.error("Failed to get TEE agent:", error);
            res.status(500).json({
    ...
    router.post(
        ...
        async (req: express.Request, res: express.Response) => {
            try {
                ...
                ronsLogger.error("Failed to get TEE logs:", error);
                res.status(500).json({
