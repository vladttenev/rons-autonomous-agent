...
export class DirectClient {
    ...
    constructor() {
        ronsLogger.log("DirectClient constructor");
        this.app = express();
        this.app.use(cors());
        this.agents = new Map();
        ...
        this.app.get(
            ...
            async (req: express.Request, res: express.Response) => {
                ...
                ronsLogger.log("Download directory:", downloadDir);

                try {
                    ronsLogger.log("Creating directory...");
                    await fs.promises.mkdir(downloadDir, { recursive: true });

                    ronsLogger.log("Fetching file...");
                    ...
                    ronsLogger.log("Response headers:", fileResponse.headers);
                    ...
                    ronsLogger.log("Saving as:", fileName);

                    const arrayBuffer = await fileResponse.arrayBuffer();
                    const buffer = Buffer.from(arrayBuffer);

                    const filePath = path.join(downloadDir, fileName);
                    ronsLogger.log("Full file path:", filePath);
                    ...
                    ronsLogger.error("Detailed error:", error);
                    res.status(500).json({
    ...
    public start(port: number) {
        ...
        const gracefulShutdown = () => {
            ronsLogger.log("Received shutdown signal, closing server...");
            this.server.close(() => {
                ronsLogger.success("Server closed successfully");
                process.exit(0);
            });
    ...
    public stop() {
        if (this.server) {
            this.server.close(() => {
                ronsLogger.success("Server stopped");
            });
        }
    }
}
...
export const DirectClientInterface: Client = {
    start: async (_runtime: IAgentRuntime) => {
        ronsLogger.log("DirectClientInterface start");
