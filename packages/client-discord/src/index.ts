...
export class DiscordClient extends EventEmitter {
    ...
    async stop() {
        try {
            ...
            ronsLogger.error("client-discord instance stop err", e);
        }
    }

    private async onClientReady(readyClient: { user: { tag: any; id: any } }) {
        ronsLogger.success(`Logged in as ${readyClient.user?.tag}`);
        ...
        try {
            ...
            ronsLogger.success("Slash commands registered");
        } catch (error) {
        ...
        ronsLogger.success("Use this URL to add the bot to your server:");
        ronsLogger.success(
    ...
    async handleReactionAdd(reaction: MessageReaction, user: User) {
        try {
            ronsLogger.log("Reaction added");

            // Early returns
            if (!reaction || !user) {
                ronsLogger.warn("Invalid reaction or user");
                return;
            }
            ...
            if (!userIdUUID || !roomId) {
                ronsLogger.error("Invalid user ID or room ID", {
                    userIdUUID,
                    roomId,
                });
                return;
            ...
            try {
                ...
                ronsLogger.debug("Reaction memory created", {
                ...
                if (error.code === "23505") {
                    // Duplicate key error
                    ronsLogger.warn("Duplicate reaction memory, skipping", {
                        reactionId: reactionUUID,
                    });
                    return;
                }
            ...
            ronsLogger.error("Error handling reaction:", error);
        }
    }

    async handleReactionRemove(reaction: MessageReaction, user: User) {
        ronsLogger.log("Reaction removed");
        // if (user.bot) return;
...
export const DiscordClientInterface: RonsClient = {
    start: async (runtime: IAgentRuntime) => new DiscordClient(runtime),
    stop: async (runtime: IAgentRuntime) => {
        try {
            // stop it
            ronsLogger.log("Stopping discord client", runtime.agentId);
            await runtime.clients.discord.stop();
        } catch (e) {
            ronsLogger.error("client-discord interface stop error", e);
        }
    },
};
