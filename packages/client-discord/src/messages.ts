import { composeContext, composeRandomUser } from "@rons-org/core";
import { generateMessageResponse, generateShouldRespond } from "@rons-org/core";
import {
    type Content,
...
import { stringToUuid, getEmbeddingZeroVector } from "@rons-org/core";
import {
    ChannelType,
    type Client,
...
export class MessageManager {
    ...
    private _startAutoPostMonitoring(): void {
        ...
        if (!this.client.isReady()) {
            ronsLogger.info('[AutoPost Discord] Client not ready, waiting for ready event')
            this.client.once('ready', () => {
                ronsLogger.info('[AutoPost Discord] Client ready, starting monitoring')
                this._initializeAutoPost();
            });
        } else {
            ronsLogger.info('[AutoPost Discord] Client already ready, starting monitoring')
            this._initializeAutoPost();
        }
    }
        ...
        try {
            ...
            if ((timeSinceLastMessage > randomThreshold) &&
                ...
                try {
                    ...
                    ronsLogger.warn("[AutoPost Discord] Error:", error);
                }
            } else {
                ronsLogger.warn("[AutoPost Discord] Activity within threshold. Not posting.");
            }
        } catch (error) {
            ronsLogger.warn("[AutoPost Discord] Error checking last message:", error);
        }
    }
    ...
    private async _monitorAnnouncementChannels(): Promise<void> {
        if (!this.autoPostConfig.enabled || !this.autoPostConfig.announcementChannelIds.length) {
            ronsLogger.warn('[AutoPost Discord] Auto post config disabled or no announcement channels')
            return;
        }
                    ...
                    try {
                        newsChannel.createMessageCollector().on('collect', async (message: DiscordMessage) => {
                            ...
                            try {
                                ...
                                ronsLogger.warn("[AutoPost Discord] Announcement Error:", error);
                            }
                        });
                        ronsLogger.info(`[AutoPost Discord] Successfully set up collector for announcement channel: ${newsChannel.name}`);
                    } catch (error) {
                        ronsLogger.warn(`[AutoPost Discord] Error setting up announcement channel collector:`, error);
                    }
                } else {
                    ronsLogger.warn(`[AutoPost Discord] Channel ${announcementChannelId} is not a valid announcement or text channel, type:`, channel.type);
                }
            } else {
                ronsLogger.warn(`[AutoPost Discord] Could not find channel ${announcementChannelId} directly`);
            }
        }
    }
    ...
    private async _shouldIgnore(message: DiscordMessage): Promise<boolean> {
        ...
        // short responses where rons should stop talking and disengage unless mentioned again
        if (
    ...
    private async _shouldRespond(
        ...
        try {
            ...
            ronsLogger.error("Error in _shouldRespond team processing:", {
                error,
                agentId: this.runtime.agentId,
