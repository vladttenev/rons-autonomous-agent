...
In the Rons framework, an agent consists of three key components: a brain (accessing an LLM), actions (the tasks the
...
export const postTweetAction: Action = {
  ...
  validate: async (
    ...
    ronsLogger.log(`Has credentials: ${hasCredentials}`);
...
```bash
pnpm add @rons-org/plugin-primus
```
...
To use the plugin, add `@rons-org/plugin-primus` to the plugins field in your character file. Here's an example of how your character file might look after the update:
