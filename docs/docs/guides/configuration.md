...
This guide covers how to configure Rons for different use cases and environments. We'll walk through all available configuration options and best practices.
...
Rons supports routing API calls through [Cloudflare AI Gateway](https://developers.cloudflare.com/ai-gateway/), which provides several benefits:
...
When enabled, Rons will automatically route requests through your Cloudflare AI Gateway endpoint. The gateway URL is constructed in the format:
```
https://gateway.ai.cloudflare.com/v1/${accountId}/${gatewayId}/${provider}
```

If the gateway configuration is incomplete or disabled, Rons will fall back to direct API calls.
...
3. Configure in `ronsConfig.yaml`:
...
Enable and configure plugins in `ronsConfig.yaml`:
