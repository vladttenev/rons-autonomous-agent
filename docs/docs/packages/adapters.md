...
Database Adapters provide Rons's persistence layer, enabling storage and retrieval of memories, relationships, goals, and other data through a unified interface. The system supports multiple database backends:

### Available Adapters

Each adapter is optimized for different use cases:

- **PostgreSQL** (`@rons-org/adapter-postgres`)

    - Production-ready with vector search
    - Connection pooling and high performance
    - JSONB and pgvector support

- **SQLite** (`@rons-org/adapter-sqlite`)

    - Lightweight local development
    - No external dependencies
    - Full-text search capabilities

- **Supabase** (`@rons-org/adapter-supabase`)

    - Cloud-native PostgreSQL
    - Real-time subscriptions
    - Built-in RPC functions

- **SQL.js** (`@rons-org/adapter-sqljs`)
...
Rons's database adapters provide a unified interface for data persistence across different storage backends. The following diagram shows how adapters integrate with the system:
...
```bash
# PostgreSQL
pnpm add @rons-org/adapter-postgres pg

# SQLite
pnpm add @rons-org/adapter-sqlite better-sqlite3

# SQL.js
pnpm add @rons-org/adapter-sqljs sql.js

# Supabase
pnpm add @rons-org/adapter-supabase @supabase/supabase-js

# PgLite
pnpm add @rons-org/adapter-pglite @electric-sql/pglite
```
