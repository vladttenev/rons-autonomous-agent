...
The Agent Package (`@rons/agent`) provides the high-level orchestration layer for Rons, managing agent lifecycles, character loading, client initialization, and runtime coordination.
...
The Agent Package (`@rons-org/agent`) serves as the orchestration layer for Rons, handling:
...
```bash
pnpm add @rons-org/agent
```

## Quick Start

```typescript
import { startAgents, loadCharacters } from "@rons-org/agent";
