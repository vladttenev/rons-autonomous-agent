...
Database Adapters provide the persistence layer for Rons, enabling storage and retrieval of memories, relationships, goals, and other core data. The system supports multiple database backends through a unified interface.

## Available Adapters

Rons includes the following database adapters:

- **PostgreSQL Adapter** (`@rons/adapter-postgres`) - Production-ready adapter for PostgreSQL databases
- **SQLite Adapter** (`@rons/adapter-sqlite`) - Lightweight adapter for SQLite, perfect for development
- **SQL.js Adapter** (`@rons/adapter-sqljs`) - In-memory SQLite adapter for testing
- **Supabase Adapter** (`@rons/adapter-supabase`) - Cloud-native adapter for Supabase

## Installation

```bash
# PostgreSQL
pnpm add @rons/adapter-postgres

# SQLite
pnpm add @rons/adapter-sqlite

# SQL.js
pnpm add @rons/adapter-sqljs

# Supabase
pnpm add @rons/adapter-supabase
```
...
```typescript
import { SqliteDatabaseAdapter } from "@rons/adapter-sqlite";
...
```typescript
import { PostgresDatabaseAdapter } from "@rons/adapter-postgres";
...
```typescript
import { SupabaseDatabaseAdapter } from "@rons/adapter-supabase";
...
Memories are the fundamental unit of storage in Rons. They represent messages, documents, and other content with optional embeddings for semantic search.
