# Rons - Multi-agent simulation framework

# https://github.com/ronsOS/rons

# Visit https://rons.builders for support
...
- Highly extensible - create your own actions and clients to extend Rons's capabilities
...
To avoid git clashes in the core directory, we recommend adding custom actions to a `custom_actions` directory and then adding them to the `ronsConfig.yaml` file. See the `ronsConfig.example.yaml` file for an example.
...
The docusaurus server will get started and you can verify it locally at https://localhost:3000/rons.
