...
date: 2024-12-31
description: "Meeting notes covering RonsOS's contributor discussions on improving developer onboarding, CI/CD pipelines, funding models, and technical improvements to the core platform."
---
...
This was a weekly contributor meeting for RonsOS, an open-source project by ai16z building an AI agent framework.
...
*   They discussed the location of embedding-related code in the Rons codebase (searching for `get_remote_embeddings`) and the issue of zero vectors being used in place of actual data.
*   There was a brief discussion about how to feed external data (PDFs, Markdown files) into Rons for context, with a mention of the `knowledge` field in character files and the limitations of the current room-based scoping of knowledge.
