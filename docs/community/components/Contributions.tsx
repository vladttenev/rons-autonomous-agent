    ...
    const fetchCommits = async (page: number) => {
        try {
            const commitResponse = await fetch(
                `https://api.github.com/repos/rons-org/rons/commits?author=${contributor.login}&page=${page}`,
                {
                    method: "GET",
                    headers: {
    ...
    const fetchPRs = async (page: number) => {
        try {
            const prResponse = await fetch(
                `https://api.github.com/search/issues?q=type:pr+author:${contributor.login}+repo:rons-org/rons&page=${page}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: `token ${githubAccessToken}`,
    ...
    const fetchIssues = async (page: number) => {
        try {
            const issueResponse = await fetch(
                `https://api.github.com/search/issues?q=type:issue+author:${contributor.login}+repo:rons-org/rons&page=${page}`,
                {
                    method: "GET",
                    headers: {
                        Authorization: `token ${githubAccessToken}`,
