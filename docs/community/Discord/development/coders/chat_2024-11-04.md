...
- How do I switch the bot model to Claude in Rons?
...
- Agent Joshua ₱ helped Ophiuchus with testing a Docker image for Rons by planning to add a Dockerfile, which would allow him to test it in a real TEE Server environment. He also shared his progress on building a next.js example app that can make remote attestations and derive ecdsa keypairs.
...
- Technical Tasks
- Build a docker image for Rons (mentioned by Agent Joshua)
