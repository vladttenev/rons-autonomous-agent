...
- When running `pnpm add -D ts-node typescript @types/node --filter '@rons-org/agent'` , it outputs 'No projects matched filters'. What should I do? Thanks! (asked by Harvz)
...
- Does Rons/Spartan have public endpoint which can be used to integrate them into an app? (asked by @Ancosero (05:26))
- How do I change the model being used by Anthropic on Rons, like switching between sonnet and opus? (asked by @Thebasement (06:14))
...
- (asked by @Odilitime)
- Has anyone built RAG with Rons? Who can help me get started on this project? (asked by @hajmamad)
...
- @Ladi helped All members with Documentation Needs by providing Fixing missing scripts for @rons-org/plugin-node installation
...
- Switch from version v0.1.5 of Rons codebase to v0.1-alpha3, as it appears more stable. (mentioned by Coelacanth)
...
- Investigate missing scripts/postinstall.js for @rons-org/plugin-node installation (mentioned by @Ladi)
...
- Integrate Rons/Spartan with public endpoint in app development (mentioned by @Ancosero (05:26))
