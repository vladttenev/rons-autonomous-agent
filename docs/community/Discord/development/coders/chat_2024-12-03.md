...
- Nona (ag/acc) helped Tharakesh with Implementing function calls in AI model by providing dievardump provided guidance on integrating action calling within Rons.
...
- Implement action calling within Rons similar to 'open ai function calling' (mentioned by dievardump)
...
- Investigate issues with installing Twitter client using Rons framework (mentioned by @allan28)
...
- Resolve import error for '@rons-org/adapter-postgres', use dynamic import() instead of require in CommonJS modules. (mentioned by tybq93)
...
- Research and implement 'create wallet on TEE' feature in the newest Rons release or find alternative for storing private keys. (mentioned by @SotoAlt | WAWE)
